...
/* ── [3 set 2026] EVENTI SU PIÙ GIORNI ────────────────────────────────────
   Un evento con dataFine compare in tutti i giorni dell'intervallo. Il
   confronto è fra stringhe: le date in forma aaaa-mm-gg si ordinano già
   correttamente da sole, quindi niente conversioni e niente cicli sui giorni.
   _calSpan restituisce null quando l'evento dura un giorno solo, quando la
   fine precede l'inizio (errore di battitura) o quando l'intervallo supera
   l'anno — così un 2226 scritto per sbaglio non genera un evento infinito. */
...
function _calSpan(e){
  const d1 = String(e && e.data || '').trim();
  const d2 = String(e && e.dataFine || '').trim();
  if(!d1 || !d2 || d2 <= d1) return null;
  const tot = _calSpanGiorni(d1, d2);
  if(!tot || tot < 2 || tot > 366) return null;
  return {inizio:d1, fine:d2, tot:tot};
}
/* Vero se il giorno ds è coperto dall'evento (giorno singolo o intervallo). */
function _calCopre(e, ds){
  if(e.data === ds) return true;
  const sp = _calSpan(e);
  return !!(sp && ds > sp.inizio && ds <= sp.fine);
}
