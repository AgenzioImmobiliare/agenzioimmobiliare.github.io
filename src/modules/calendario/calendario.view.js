...
function calRenderSettimana(og,mesi){
  ...
  /* ── [11 set 2026] LE FASCE SI ADATTANO ALLA SETTIMANA ──────────────────
     Prima erano sempre 07:00-22:00: sedici fasce da 56 pixel fanno 896 pixel,
     dentro un riquadro alto 520 che scorre. Si vedeva fino alle 16:00 e il
     resto restava sotto — ed è lì che stanno quasi tutti gli appuntamenti.
     Ho provato due volte a portare lo scorrimento sul primo evento e due
     volte qualcosa lo ha rimesso a zero: invece di insistere, tolgo il
     bisogno di scorrere.
     Ora la griglia parte un'ora prima del primo appuntamento e finisce un'ora
     dopo l'ultimo, con un minimo di dieci fasce perché non sembri schiacciata.
     Settimana vuota: orario di lavoro normale, 08:00-20:00. */
  var _oreSett=[];
  giorni.forEach(function(d){
    (calGetEventi(toDs(d))||[]).forEach(function(e){
      var h=parseInt(String(e&&e.ora||'').slice(0,2));
      if(!isNaN(h) && h>=0 && h<=23) _oreSett.push(h);
    });
  });
  var _da, _a;
  if(_oreSett.length){
    _da=Math.max(0, Math.min.apply(null,_oreSett)-1);
    _a =Math.min(23, Math.max.apply(null,_oreSett)+1);
  } else { _da=8; _a=20; }
  /* Per arrivare alle dieci fasce minime si allarga PRIMA verso il mattino e
     dentro l'orario di lavoro (07-22): con appuntamenti alle 17-19 è meglio
     vedere 11:00-20:00 che 14:00-23:00, che sembra una griglia notturna. */
  var _giri=0;
  while((_a-_da+1)<10 && _giri++<30){
    if(_da>7) _da--;
    else if(_a<22) _a++;
    else if(_da>0) _da--;
    else if(_a<23) _a++;
  ...
  const slots=[];for(let h=_da;h<=_a;h++) slots.push(String(h).padStart(2,'0')+':00');
  const area=document.getElementById('cal-view-area');
  if(!area) return;
  area.innerHTML=`<div style="background:white;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08);border:1px solid var(--border)">
    ...
    <div id="cal-sett-griglia" style="display:flex;overflow-y:auto;max-height:min(640px,66vh)">
      ...
      ${giorni.map((d,gi)=>{const ds=toDs(d);const evs=calGetEventi(ds);const isDom=gi===6;return`<div style="flex:1;min-width:0;overflow:hidden;border-left:1px solid ${gi===0?'transparent':'var(--border)'};background:${isDom?'#FAFAFA':'white'}">
          ${slots.map((h,hi)=>{const hEvs=evs.filter(e=>(e.ora||'').startsWith(String(_da+hi).padStart(2,'0')));return`<div style="height:56px;border-bottom:1px solid #F1F5F9;padding:2px 2px;overflow:hidden;box-sizing:border-box" ondblclick="openEventoPre('${ds}')">
  ...
  calClick(og);
  /* [10 set 2026] La griglia settimanale è alta 520px e scorre: a 56 pixel per
     fascia se ne vedono nove, cioè dalle 07:00 alle 16:00. Tutto quello che
     succede dal tardo pomeriggio in poi restava sotto, e sembrava che gli
     appuntamenti non ci fossero — mentre l'intestazione li contava.
     Ora la griglia si porta da sola sul primo appuntamento della settimana,
     con un'ora di margine sopra per far vedere che prima non c'è niente.
     Se la settimana è vuota resta all'inizio. */
  /* Niente più scorrimento automatico: ora le fasce ci stanno tutte. */
}

function _calPortaSuPrimoEvento(giorni, toDs){
  /* Prima versione: impostavo scrollTop subito dopo aver scritto la griglia.
     Non funzionava, e il motivo è che in quel momento il browser non l'ha
     ancora disposta a schermo: su un elemento con altezza zero lo scorrimento
     viene semplicemente ignorato, senza errori.
     Ora si aspetta che la griglia sia davvero misurabile e scorribile, e si
     riprova qualche volta a brevissima distanza. */
  var ALTEZZA=56, PRIMA=7;   /* stessa misura e stessa ora d'inizio della griglia */
  var primaOra=null;
  (giorni||[]).forEach(function(d){
    (calGetEventi(toDs(d))||[]).forEach(function(e){
      var h=parseInt(String(e&&e.ora||'').slice(0,2));
      if(!isNaN(h) && (primaOra===null || h<primaOra)) primaOra=h;
    });
  });
  if(primaOra===null) return;                       /* settimana senza orari: si resta in cima */
  var riga=Math.max(0, primaOra-PRIMA-1);           /* un'ora di margine sopra */
  var meta=riga*ALTEZZA;
  if(meta<=0) return;

  var tentativi=0;
  function prova(){
    tentativi++;
    var box=document.getElementById('cal-sett-griglia');
    /* pronta = esiste, è visibile (ha un'altezza) e ha davvero qualcosa da
       scorrere. Finché non lo è, riprovare non costa niente. */
    var pronta = box && box.clientHeight>0 && box.scrollHeight>box.clientHeight+4;
    if(pronta){
      try{ box.scrollTop=meta; }catch(e){}
      /* Verifica: se il browser non ha accettato il valore si riprova ancora. */
      if(Math.abs(box.scrollTop-meta)<6 || tentativi>=8) return;
    }
    if(tentativi<8) setTimeout(prova, 90);
  }
  if(typeof requestAnimationFrame==='function') requestAnimationFrame(prova);
  else setTimeout(prova,0);
}
