...
function calGetEventi(ds){
  return [
    ...D.eventi.filter(e=>e.data===ds&&!isEvVendutoOrArchiviato(e)).map(e=>({
      ...e, _type:'evento', _evIdx:D.eventi.indexOf(e)
    })),
...
function calApriEvento(tipo, idx){
  try{
    if(tipo==='evento'   && typeof editEvento==='function'){ editEvento(idx); return; }
    if(tipo==='visita'   && typeof openVisita==='function'){ openVisita(idx); return; }
    if(tipo==='incarico' && typeof openImmobileModal==='function'){ openImmobileModal(idx); return; }
    if(tipo==='scadenza' && typeof openPratica==='function'){ openPratica(idx); return; }
    /* fallback: se il tipo non è gestito, non fare nulla di distruttivo */
...
function calMostraGiorno(ds){
  ...
  const rows=evs.map(e=>{
    ...
    const idx=(e._type==='evento')?e._evIdx:(e._type==='visita')?e._visIdx:(e._type==='incarico')?e._immIdx:(e._type==='scadenza')?e._pratIdx:-1;
    const canEdit=idx>=0 && e._type;
    return `<div ${canEdit?`ondblclick="calApriEvento('${e._type}',${idx});document.getElementById('cal-giorno-modal').style.display='none'" style="cursor:pointer"`:''}
...
function _evTappaAdd(presetIdx, presetOra){
  ...
  row.innerHTML = ''
    ...
    + '<button type="button" onclick="this.closest(\'.ev-tappa-row2\').remove()" style="flex:0 0 32px;height:32px;border:1px solid #FCA5A5;background:#FEF2F2;color:#DC2626;border-radius:7px;font-weight:700;cursor:pointer;">✕</button>';
...
try{ window._evTappaAdd = _evTappaAdd; window._evTappaPickerToggle = _evTappaPickerToggle; window._evTappaPickerSelect = _evTappaPickerSelect; window._evTipoChange = _evTipoChange; }catch(e){}
function openEvento(){clearModal('modal-evento');D.editIdx=null;D.editType=null;D.editEventoId=null;document.getElementById('mt-ev').textContent='Nuovo Appuntamento';document.getElementById('ev-data').value=today();var _rw=document.getElementById('ev-tappe-rows');if(_rw)_rw.innerHTML='';var _tw=document.getElementById('ev-tappe-wrap');if(_tw)_tw.style.display='none';openModal('modal-evento');}
function openEventoPre(d){clearModal('modal-evento');D.editIdx=null;D.editType=null;D.editEventoId=null;document.getElementById('ev-data').value=d;var _rw=document.getElementById('ev-tappe-rows');if(_rw)_rw.innerHTML='';var _tw=document.getElementById('ev-tappe-wrap');if(_tw)_tw.style.display='none';openModal('modal-evento');}
...
} else D.eventi.push(ev);saveD();closeModal('modal-evento');D.editIdx=null;D.editType=null;D.editEventoId=null;renderCal();updateBadges();if(typeof renderPratiche==='function')renderPratiche();}
