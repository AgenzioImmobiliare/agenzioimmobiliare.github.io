...
function openEvento(){clearModal('modal-evento');D.editIdx=null;D.editType=null;D.editEventoId=null;document.getElementById('mt-ev').textContent='Nuovo Appuntamento';document.getElementById('ev-data').value=today();openModal('modal-evento');}
function openEventoPre(d){clearModal('modal-evento');D.editIdx=null;D.editType=null;D.editEventoId=null;document.getElementById('ev-data').value=d;openModal('modal-evento');}
...
function editEvento(id){
  ...
  clearModal('modal-evento');D.editIdx=i;D.editType='evento';D.editEventoId=id;const ev=D.eventi[i];['titolo','tipo','data','ora','cliente','tel','note'].forEach(k=>{const el=document.getElementById('ev-'+k);if(el)el.value=ev[k]||'';});openModal('modal-evento');
}
function saveEvento(){
  ...
  const g=id=>document.getElementById(id).value;const ev={titolo:g('ev-titolo'),tipo:g('ev-tipo'),data:g('ev-data'),ora:g('ev-ora'),cliente:g('ev-cliente'),tel:g('ev-tel'),note:g('ev-note')};if(!ev.titolo||!ev.data){dlgAlert('Inserisci titolo e data obbligatori.','','Campi mancanti');return;}if(D.editIdx!==null&&D.editType==='evento'){
