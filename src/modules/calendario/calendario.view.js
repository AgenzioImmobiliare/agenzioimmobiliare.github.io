...
function _evTappaOptionsHtml(selectedIdx){
  ...
  var opts = '<option value="">— Seleziona immobile —</option>';
  attivi.forEach(function(o){
    var im = o.im;
    var lbl = (im.tipo||'Immobile')+(im.ref?' · '+im.ref:'')+(im.comune?' — '+im.comune:'');
    opts += '<option value="'+o.i+'"'+(String(selectedIdx)===String(o.i)?' selected':'')+'>'+escH(lbl)+'</option>';
  });
  return opts;
}
function _evTappaAdd(presetIdx, presetOra){
  ...
  row.className = 'ev-tappa-row';
  row.innerHTML = ''
    + '<select class="fselect ev-tappa-imm">'+_evTappaOptionsHtml(presetIdx)+'</select>'
    + '<input type="time" class="finput ev-tappa-ora" value="'+(presetOra||'')+'">'
...
try{ window._evTappaOptionsHtml = _evTappaOptionsHtml; window._evTappaAdd = _evTappaAdd; window._evTipoChange = _evTipoChange; }catch(e){}
...
function saveEvento(){
  ...
  if(ev.tipo==='visita'){
    var _tappeS=[];
    document.querySelectorAll('#ev-tappe-rows .ev-tappa-row').forEach(function(row){
      var sel=row.querySelector('.ev-tappa-imm'); var ora=row.querySelector('.ev-tappa-ora');
      var v = sel ? sel.value : '';
