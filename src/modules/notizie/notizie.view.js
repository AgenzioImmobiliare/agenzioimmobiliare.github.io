...
function delNotizia(idxOrId){
  ...
  if(confirm('Eliminare questa notizia?')){
    ...
    if(iNow<0) return;
    D.notizie.splice(iNow,1);saveD();renderNotizie();updateBadges();
  }
}
...
function convNotizia(idxOrId){
  ...
  dlgConfirm(
    ...
    const cl={
      ...
      tel:nFresh.tel||'',
      email:nFresh.email||'',
      tipo:nFresh.tipo==='Acquisto'?'acquirente':'venditore',
      fonte:nFresh.modalita||nFresh.fonte||'',
      citta:nFresh.comune||'',
      zona:nFresh.zona||'',
      op:nFresh.tipo||'',
