...
function renderAttivita(){
  ...
  tbody.innerHTML = lista.map(function(a){
    ...
    if(!a.id) a.id=(typeof genUUID==='function')?genUUID():('att_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));
    ...
    return '<tr style="'+rowBg+'">'
      ...
      +'<td><div class="actions-col" style="gap:4px">'
        +(a.stato!=='Completata'?'<button class="icon-btn" onclick="attSegnaCompletata(\''+aId+'\')" title="Segna completata" style="color:#15803D">'+svgDone+'</button>':'')
        +'<button class="icon-btn" onclick="openAttivita(\''+aId+'\')" title="Modifica">'+svgEdit+'</button>'
        +'<button class="icon-btn" onclick="delAttivita(\''+aId+'\')" style="color:var(--red-l)" title="Elimina">'+svgDel+'</button>'
      +'</div></td>'
    +'</tr>';
  }).join('');
}
...
function openAttivita(idxOrId){
  _attEnsure();
  var idx=idxOrId!==undefined?(typeof idxOrId==='string'?D.attivita.findIndex(function(x){return x&&x.id===idxOrId;}):idxOrId):undefined;
  if(idxOrId!==undefined && (idx===undefined || idx<0)){
    if(typeof showToast==='function') showToast('Attività non trovata (forse aggiornata altrove). Aggiorno la lista...','','#DC2626');
    try{ renderAttivita(); }catch(e){}
    return;
  }
  D.editIdx=null; D.editType=null; D.editAttivitaId=null;
  ...
  if(idx!==undefined){
    if(!D.attivita[idx].id) D.attivita[idx].id=(typeof genUUID==='function')?genUUID():('att_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));
    D.editIdx=idx; D.editType='attivita'; D.editAttivitaId=D.attivita[idx].id;
...
function saveAttivita(){
  _attEnsure();
  /* Anti-drift: ri-risolvi l'indice reale dell'attività in modifica appena
     prima di leggere/scrivere D.attivita. */
  if(D.editType==='attivita' && D.editAttivitaId){
    var _freshIdx=D.attivita.findIndex(function(x){return x&&x.id===D.editAttivitaId;});
    if(_freshIdx<0){
      alert("Questa attività non esiste più (probabilmente cancellata o aggiornata da un altro dispositivo). La lista viene aggiornata.");
      D.editIdx=null; D.editType=null; D.editAttivitaId=null;
      try{ closeModal('modal-attivita'); }catch(_e){}
      renderAttivita();
      return;
    }
    D.editIdx=_freshIdx;
  ...
  if(D.editIdx!==null && D.editType==='attivita'){
    a.id=D.editAttivitaId||D.attivita[D.editIdx].id||((typeof genUUID==='function')?genUUID():('att_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8)));
    D.attivita[D.editIdx]=a;
  } else {
    a.id=(typeof genUUID==='function')?genUUID():('att_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));
    D.attivita.push(a);
  }
  saveD();
...
function delAttivita(idxOrId){
  _attEnsure();
  var i0=typeof idxOrId==='string'?D.attivita.findIndex(function(x){return x&&x.id===idxOrId;}):idxOrId;
  var a0=D.attivita[i0];
  if(!a0){
    if(typeof showToast==='function') showToast('Attività non trovata (forse già cancellata altrove). Aggiorno la lista...','','#DC2626');
    try{ renderAttivita(); }catch(e){}
    return;
  }
  if(!a0.id) a0.id=(typeof genUUID==='function')?genUUID():('att_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));
  var aId=a0.id; /* catturato ORA: la cancellazione avviene dopo la conferma, che richiede tempo */
  dlgConfirm("Eliminare l'attività <strong>"+(a0.oggetto||'')+"</strong>?",'','Elimina').then(function(ok){
    if(!ok) return;
    /* Anti-drift: ri-risolvi l'indice fresco dall'id proprio ora che
       l'utente ha confermato. */
    var i=D.attivita.findIndex(function(x){return x&&x.id===aId;});
    if(i<0){
      if(typeof showToast==='function') showToast('Attività già cancellata o aggiornata altrove. Aggiorno la lista...','','#DC2626');
      renderAttivita();
      return;
    }
    D.attivita.splice(i,1);
    /* Segna come cancellata "per sempre" (id o chiave composita, come mobile). */
    try{ if(typeof _attAddDeletedId==='function') _attAddDeletedId(a0||aId); }catch(_e){}
    saveD();
    /* Push immediato dedicato (non aspetta il debounce di 1-2s): una
       cancellazione è importante da non perdere se chiudi subito la vista. */
    try{ if(typeof _attForcePushNow==='function') _attForcePushNow(); else if(typeof window._forcePushNow==='function') window._forcePushNow(); }catch(_e){}
    renderAttivita();
    showToast('Attività eliminata','','#DC2626');
  });
}

function attSegnaCompletata(idxOrId){
  _attEnsure();
  var idx=typeof idxOrId==='string'?D.attivita.findIndex(function(x){return x&&x.id===idxOrId;}):idxOrId;
  var a=D.attivita[idx];
  if(!a) return;
  a.stato='Completata';
