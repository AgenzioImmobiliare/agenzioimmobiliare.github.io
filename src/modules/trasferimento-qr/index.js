/* ═══════════════════════════════════════════════════════════════════════
   ...
   Trasferimento veloce FOTO da TELEFONO a PC via QR code.
   ---------------------------------------------------------------------
   VERSO (importante — il PC di Enzo NON ha webcam):
   - Il QR si GENERA sul PC.
   - Il telefono INQUADRA il QR con la sua fotocamera.
   - Si apre un canale diretto: il telefono INVIA le foto selezionate,
     il PC le RICEVE e le salva in una cartella scelta ogni volta.
   - I dati testuali NON servono qui: Enzo li ha già sul PC. Solo foto.
   ---------------------------------------------------------------------
   Nessuna foto passa da Firebase o altri cloud:
   - Telefono e PC si "agganciano" tramite il Worker Cloudflare esistente
     (stesso Worker di callClaude, endpoint /signal/...): vi transita solo
     un piccolo messaggio tecnico (offerta/risposta WebRTC), non le foto.
   - Una volta agganciati, le foto viaggiano DIRETTAMENTE dal telefono al
     PC (WebRTC DataChannel), tipicamente sulla stessa rete WiFi.
...
(function () {
  ...
  /* ── Rileva se siamo su telefono (fotocamera utile) o su PC ────────── */
  function tqIsMobile() {
    return /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent);
  }

  /* ── Stato condiviso della sessione corrente ──────────────────────── */
  var tqStato = {};
  function tqReset() {
  ...
  window.renderTrasferimentoQR = function () {
    ...
    if (!el) return;
    tqReset();
    var mobile = tqIsMobile();
    el.innerHTML = ''
      + '<div class="print-header"><div class="ph-wrap"><div class="ph-agency">Le case dalla A allo Z.io</div><div class="ph-line2"><span class="ph-agent">Agente: Vincenzo Carnicelli</span></div><div class="ph-section">Trasferimento Foto (QR)</div></div></div>'
      + '<div style="max-width:720px;margin:0 auto;">'
      + '<h2 style="margin:0 0 4px;display:flex;align-items:center;gap:8px;font-size:1.15rem;font-weight:800;color:var(--text)">' + tqIcon('M3 11l19-9-9 19-2-8-8-2z') + ' Trasferimento Foto da Telefono a PC</h2>'
      + '<p style="color:var(--text3);font-size:0.85rem;margin:0 0 18px;">Genera il QR sul PC, inquadralo col telefono e invia le foto: arrivano direttamente in una cartella del PC, senza cloud di mezzo.</p>'
      + '<div style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap">'
      + '  <button class="btn ' + (mobile ? 'btn-outline' : 'btn-primary') + '" style="flex:1;min-width:220px" onclick="tqAvviaRicezionePC()">' + tqIcon('M4 5h16v10H4z M2 19h20 M9 9h6') + ' Sono al PC: genera QR</button>'
      + '  <button class="btn ' + (mobile ? 'btn-primary' : 'btn-outline') + '" style="flex:1;min-width:220px" onclick="tqAvviaInvioTelefono()">' + tqIcon('M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z M12 17a4 4 0 100-8 4 4 0 000 8z') + ' Sono al telefono: invia foto</button>'
      + '</div>'
      + '<div id="tq-area"></div>'
      + '</div>';
  };
  ...
  /* ═══════════════════════════════════════════════════════════════════
     LATO PC — genera il QR e resta in ascolto per ricevere le foto
     ═══════════════════════════════════════════════════════════════════ */
  window.tqAvviaRicezionePC = function () {
    tqReset();
    var area = document.getElementById('tq-area');
    if (!area) return;
    area.innerHTML = '<div id="tq-pc-esito" style="text-align:center;color:var(--text3);font-size:0.85rem">Preparazione del QR…</div>';
    ...
    pc.createOffer().then(function (offer) {
      ...
      return tqSignalPut(room, 'offer', pc.localDescription.sdp).then(function () {
        var dataUrl = (typeof qrDataUrl === 'function') ? qrDataUrl(room, 240) : null;
        var esito = document.getElementById('tq-pc-esito');
        if (esito) {
          esito.innerHTML = ''
            + '<div style="font-size:0.9rem;color:var(--text);margin-bottom:10px">Inquadra questo QR con il telefono:</div>'
            ...
            + '<div style="font-size:0.78rem;color:var(--text3);margin-top:4px">Se il QR non si legge, digita questo codice sul telefono.</div>'
            + '<div id="tq-pc-stato" style="margin-top:14px;font-size:0.85rem;color:var(--text3)">In attesa che il telefono si colleghi…</div>';
        }
  ...
  function tqAvviaPollingAnswer(room, pc) {
    ...
    tqStato.pollTimer = setInterval(function () {
      if (Date.now() > scaduto) {
        ...
        var st = document.getElementById('tq-pc-stato');
        if (st) st.innerHTML = '<span style="color:#DC2626">Tempo scaduto: nessun telefono si è collegato. Genera un nuovo QR.</span>';
        return;
      }
      tqSignalGet(room, 'answer').then(function (sdp) {
        ...
        pc.setRemoteDescription({ type: 'answer', sdp: sdp }).then(function () {
          var st = document.getElementById('tq-pc-stato');
          if (st) st.textContent = 'Telefono collegato — in attesa delle foto…';
        });
  ...
  function tqRicezioneAscolta(dc) {
    ...
    dc.onmessage = function (ev) {
      if (typeof ev.data === 'string') {
        ...
        if (msg.tipo === 'manifest') {
          manifest = msg;
          byteTotali = (msg.foto || []).reduce(function (a, f) { return a + f.size; }, 0);
          var st0 = document.getElementById('tq-pc-stato');
          if (st0) st0.textContent = 'Ricezione di ' + (msg.foto || []).length + ' foto…';
          ...
          tqMostraRisultatoPC(fotoRicevute.filter(Boolean));
        }
      } else if (fotoInCorso) {
        fotoInCorso.chunks.push(ev.data);
        byteRicevuti += ev.data.byteLength;
        var st = document.getElementById('tq-pc-stato');
        if (st && byteTotali) st.textContent = 'Ricezione foto… ' + Math.round((byteRicevuti / byteTotali) * 100) + '%';
      }
    };
  }

  function tqMostraRisultatoPC(foto) {
    var st = document.getElementById('tq-pc-stato');
    if (st) st.innerHTML = '<span style="color:#15803D">✓ ' + foto.length + ' foto ricevute.</span>';
    tqStato.ricevute = foto;
    var esito = document.getElementById('tq-pc-esito');
    if (!esito) return;
    var galleria = foto.map(function (f) {
      var url = URL.createObjectURL(f.blob);
      return '<img src="' + url + '" style="width:72px;height:72px;object-fit:cover;border-radius:8px;border:1px solid var(--border)">';
    }).join('');
    esito.innerHTML = ''
      + '<div style="font-size:0.9rem;color:#15803D;font-weight:700;margin-bottom:10px">✓ ' + foto.length + ' foto ricevute dal telefono</div>'
      + '<div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:center;margin-bottom:14px">' + galleria + '</div>'
      + '<button class="btn btn-primary" style="min-width:220px" onclick="tqSalvaFoto()">' + tqIcon('M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4 M7 10l5 5 5-5 M12 15V3') + ' Salva le ' + foto.length + ' foto sul PC</button>';
  }

  window.tqSalvaFoto = async function () {
    var foto = tqStato.ricevute || [];
    if (!foto.length) return;
    if (!window.showDirectoryPicker) {
      // Fallback per browser senza File System Access API: scarico ogni foto
  ...
  /* ═══════════════════════════════════════════════════════════════════
     LATO TELEFONO — scansiona il QR (o inserisci il codice) e invia foto
     ═══════════════════════════════════════════════════════════════════ */
  window.tqAvviaInvioTelefono = function () {
    tqReset();
    var area = document.getElementById('tq-area');
    if (!area) return;
    area.innerHTML = ''
      + '<div class="frow fcol">'
      + '  <label class="flabel">Foto da inviare</label>'
      ...
      + '  <button class="btn btn-primary" id="tq-btn-scan" onclick="tqAvviaScansione()" disabled style="min-width:240px">' + tqIcon('M3 3h7v7H3z M14 3h7v7h-7z M14 14h7v7h-7z M3 14h7v7H3z') + ' Inquadra il QR del PC</button>'
      + '  <div style="font-size:0.78rem;color:var(--text3);margin:10px 0">— oppure, se il QR non si legge —</div>'
      ...
      + '    <button class="btn btn-outline btn-sm" onclick="tqConnettiCodice()">Invia</button>'
      + '  </div>'
      + '</div>'
      + '<div style="margin:14px 0;text-align:center"><video id="tq-video" style="display:none;max-width:320px;border-radius:12px;border:1px solid var(--border)" playsinline></video></div>'
      + '<canvas id="tq-canvas" style="display:none"></canvas>'
      + '<div id="tq-inv-stato" style="margin-top:12px;text-align:center;font-size:0.85rem;color:var(--text3)"></div>';
  };
  ...
  window.tqAggiungiFoto = function (files) {
    ...
    var btn = document.getElementById('tq-btn-scan');
    if (btn) btn.disabled = tqStato.foto.length === 0;
  };
  ...
  function tqRidisegnaFotoLista() {
    ...
    cont.innerHTML = tqStato.foto.map(function (f, i) {
      ...
      return '<div style="position:relative;width:64px;height:64px">'
        ...
        + '</div>';
    }).join('') || '<span style="font-size:0.78rem;color:var(--text4)">Nessuna foto selezionata.</span>';
  }

  window.tqAvviaScansione = function () {
    if (!tqStato.foto || !tqStato.foto.length) {
      if (typeof showToast === 'function') showToast('Prima seleziona almeno una foto', '', '#DC2626');
      return;
    }
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } }).then(function (stream) {
      ...
      function tick() {
        if (video.readyState === video.HAVE_ENOUGH_DATA) {
          ...
          if (codice && codice.data) {
            ...
            tqConnettiInvia(codice.data.trim());
            return;
          }
        }
      ...
      var st = document.getElementById('tq-inv-stato');
      if (st) st.innerHTML = '<span style="color:#DC2626">Impossibile accedere alla fotocamera: ' + err.message + '</span>';
    });
  };
  ...
  window.tqConnettiCodice = function () {
    ...
    if (v.trim().length < 4) {
      if (typeof showToast === 'function') showToast('Inserisci il codice mostrato sul PC', '', '#DC2626');
      return;
    }
    tqConnettiInvia(v.trim().toUpperCase());
  };

  function tqConnettiInvia(room) {
    var st = document.getElementById('tq-inv-stato');
    if (st) st.textContent = 'Collegamento in corso…';
    tqSignalGet(room, 'offer').then(function (sdp) {
      if (!sdp) { if (st) st.innerHTML = '<span style="color:#DC2626">Codice non trovato o scaduto. Genera un nuovo QR sul PC.</span>'; return; }
      var pc = new RTCPeerConnection(tqRtcConfig());
      tqStato.pc = pc;
      var dc = pc.createDataChannel('tq');
      dc.binaryType = 'arraybuffer';
      tqStato.dc = dc;
      dc.onopen = function () { tqInviaFoto(dc); };
      ...
      pc.setRemoteDescription({ type: 'offer', sdp: sdp }).then(function () {
        ...
        if (st) st.textContent = 'Collegato — invio foto in corso…';
      }).catch(function (err) {
  ...
  function tqInviaFoto(dc) {
    ...
    function prossima() {
      if (indice >= tqStato.foto.length) {
        ...
        var st = document.getElementById('tq-inv-stato');
        if (st) st.innerHTML = '<span style="color:#15803D">✓ Foto inviate. Ora salvale sul PC.</span>';
        return;
      }
