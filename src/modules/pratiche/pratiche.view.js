...
/* [23 set 2026] L'anno NON si applica alle pratiche ancora aperte.
   Una proposta accettata a maggio 2025 che aspetta il rogito è lavoro di oggi:
   sparire dalla vista appena l'anno di lavoro passa al 2026 significava
   perderla di vista proprio mentre ci si deve stare dietro. Il filtro anno
   resta valido per "Concluse" e "Tutte", dove serve a leggere un'annata. */
function _pratAnnoSiApplica(filtro){ return filtro === 'concluse' || filtro === 'tutte'; }

function _pratAggiornaPillole(anno){
  var tutte = Array.isArray(D.pratiche) ? D.pratiche : [];
  var conta = { proposta:0, rogito:0, concluse:0, tutte:0 };
  tutte.forEach(function(p){
    var g = _pratGruppo(p);
    /* ogni pillola conta quello che vedresti aprendola: le aperte tutte,
       le concluse e "tutte" solo dell'anno scelto */
    if(g === 'proposta' || g === 'rogito'){ if(conta[g]!==undefined) conta[g]++; }
    else if(g === 'concluse'){ if(!anno || _pratAnno(p) === anno) conta.concluse++; }
    if(!anno || _pratAnno(p) === anno) conta.tutte++;
  });
...
function renderPratiche(){
  ...
  const annoSel=(document.getElementById('f-prat-anno')?.value||'');
  const anno = _pratAnnoSiApplica(_pratFiltro) ? annoSel : '';   /* aperte: nessun anno */
  _pratAggiornaPillole(annoSel);
  /* la tendina si spegne dove non ha effetto, con la spiegazione */
  try{
    var _selA=document.getElementById('f-prat-anno');
    if(_selA){
      var _off = !_pratAnnoSiApplica(_pratFiltro);
      _selA.disabled = _off;
      _selA.style.opacity = _off ? '0.45' : '';
      _selA.title = _off
        ? 'Le pratiche ancora aperte si vedono sempre, di qualunque anno: il filtro vale per Concluse e Tutte'
        : 'Mostra solo le pratiche di un anno';
    }
  }catch(e){}
