...
/* [22 ago] Toglie dagli elenchi lo stesso immobile ripetuto.
   Serve perché la scheda cliente ha, sotto ogni immobile, il pulsante di
   eliminazione: se lo stesso immobile compare due volte sembra un doppione da
   ripulire, si cancella "quello di troppo" e invece si cancella l'unico
   esistente. È già successo il 19 agosto con l'appartamento di via Madonna
   del Carmine.
   Il confronto è sull'identificativo interno (uuid); se manca si ricade
   sull'identità dell'oggetto, così due immobili diversi ma simili restano
   entrambi. Non tocca l'archivio: filtra solo quello che si mostra. */
function _immSenzaDoppioni(lista){
  if(!Array.isArray(lista)) return [];
  const visti = new Set();
  const out = [];
  for(const im of lista){
    if(!im) continue;
    const chiave = im.uuid ? 'u:'+im.uuid : im;
...
function openClienteModal(idxOrUuid){
  ...
  if(idx!==undefined){
    ...
    /* [9 set 2026] "Perché vende": gli id non seguono lo schema cli-<campo>,
       quindi vanno riempiti a parte. */
    [['cli-motivo-vendita','motivoVendita'],['cli-urgenza-vendita','urgenzaVendita'],['cli-motivo-note','motivoNote']]
      .forEach(([id,campo])=>{ const el=document.getElementById(id); if(el) el.value=c[campo]||''; });
  }
  onTipoClienteChange();
...
function onTipoClienteChange(){
  ...
  /* [9 set 2026] "Perché vende": l'opposto delle righe della richiesta —
     si vede solo per venditori e per chi è entrambe le cose. */
  ['cli-row-motivo','cli-row-fretta','cli-row-motivo-note'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.style.display = isVend ? '' : 'none';
  });
}
function saveCliente(){
  ...
  const c={motivoVendita:g('cli-motivo-vendita'),urgenzaVendita:g('cli-urgenza-vendita'),motivoNote:g('cli-motivo-note'),
    nome:g('cli-nome'),tipo:g('cli-tipo'),sesso:g('cli-sesso'),tel:g('cli-tel'),email:g('cli-email'),cf:g('cli-cf'),citta:g('cli-citta'),fonte:g('cli-fonte'),data:g('cli-data'),nascita:g('cli-nascita')};
  ...
  if(D.editIdx!==null && D.editType==='cliente'){
    ...
    /* [9 set 2026] Prima qui c'era D.clienti[D.editIdx]=c, cioè il record
       veniva SOSTITUITO per intero. Il modulo ricostruisce dieci campi, quindi
       ogni modifica cancellava tutto il resto: crmNote, foto, planimetrie,
       note, i contrassegni di archiviazione. Bastava correggere un numero di
       telefono per perdere i documenti allegati al cliente.
       aggiornaRecord parte dal record che c'è e ci scrive sopra solo quello
       che il modulo ha davvero letto. */
    D.clienti[D.editIdx]=(typeof aggiornaRecord==='function')
      ? aggiornaRecord(D.clienti[D.editIdx], c)
      : c;
    savedIdx=D.editIdx;
  } else {
    c.uuid = genUUID();
...
function archiviaCliente(idxOrUuid){
  ...
  dlgConfirm(`Archiviare il cliente "<strong>${c.nome||''}</strong>"?<br><small style="color:var(--text3)">Il cliente rimarrà nel sistema ma verrà nascosto dalla lista Attivi. Potrai riattivarlo in qualsiasi momento.</small>${extraInfo}`,'','Archivia Cliente').then(ok=>{
    ...
    immAttiviCointest.forEach(function(o){
      ...
      /* [16 set 2026] Il collegamento per uuid è quello che conta di più
         (_immApparteneCli lo guarda per primo): staccare solo quello per
         posizione lasciava l'immobile intestato al cliente appena archiviato. */
      if(im.clienteUuid && c.uuid && im.clienteUuid === c.uuid){
        im.clienteUuid = '';
      }
...
function renderClienti(){
  ...
  if(view==='grid'){
    const cards=f.map(c=>{
      ...
      /* Immobili collegati */
      const immCli=_immSenzaDoppioni(D.immobili.filter(im=>{
...
function renderPlanimetrieCli(i){
  const c=D.clienti[i];
  const storico = Array.isArray(c && c.planimetrie) ? c.planimetrie : [];
  if(!storico.length) return '';
  const nomeSafe = (c.nome||'Cliente').replace(/'/g,"\\'").replace(/"/g,'&quot;');
  let righe = storico.slice().reverse().map(function(p){
    const realIdx = storico.indexOf(p);
    const dataFmt = p.data ? new Date(p.data).toLocaleDateString('it-IT') : '';
    const nPiani = (p.dettaglio||[]).length;
    return '<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #E0F2FE;font-size:0.85rem;">'
      + '<div style="flex:1;"><b>'+dataFmt+'</b> · '+nPiani+' '+(nPiani===1?'piano':'piani')
      + ' · <b style="color:#0F2A5C;">'+Number(p.totale).toFixed(1)+' m²</b> calpestabili'
      + ' · <b style="color:#15803D;">'+Number(p.commerciale).toFixed(1)+' m²</b> commerciali</div>'
      + '<button class="btn btn-outline btn-sm" onclick="pln_stampaReportSalvato((D.clienti['+i+'].planimetrie||[])['+realIdx+'], \''+nomeSafe+'\')">🖨️ Report</button>'
      + '<button class="btn btn-outline btn-sm" onclick="pln_apriProgettoSalvato((D.clienti['+i+'].planimetrie||[])['+realIdx+'], \''+nomeSafe+'\')" title="Apri nel tool per generare la Valutazione PRO">✨ Valuta</button>'
      + '</div>';
  }).join('');
  return '<div class="card"><div class="card-header"><span class="card-title">📐 Progetti Planimetria</span>'
    + '<button class="btn btn-outline btn-sm" onclick="openPlanimetriaTool()">+ Nuova misura</button></div>'
    + '<div class="card-body">'+righe+'</div></div>';
}
...
function renderSchedaCliente(i){
  ...
  const immClienti=_immSenzaDoppioni(D.immobili.filter(im=>{
    return _immApparteneCli(im, i);
  }));
    ...
    document.getElementById('scheda-cliente-body').innerHTML=`
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          ${riqCli.map(r=>{if(!r.id)r.id=(typeof genUUID==='function')?genUUID():('riq_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));const rid=r.id.replace(/'/g,"\\'");return`<tr>
            ...
            <td><div class="actions-col"><button class="icon-btn" onclick="openRichiesta('${rid}')" title="Modifica"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button><button class="icon-btn" onclick="delRichiesta('${rid}')" style="color:var(--red-l)" title="Elimina"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg></button></div></td>
          </tr>`;}).join('')}
          </tbody></table></div>`
      ...
      ${renderPlanimetrieCli(i)}${(typeof window.renderRicercheCatCli==='function')?window.renderRicercheCatCli(c):''}
