...
function renderSchedaCliente(i){
  ...
  /* [22 ago] La barra si fermava a "Contatto" per chi aveva già rogitato.
     Il motivo: qui si guardavano solo gli stati 'acquisito', 'trattativa',
     'proposta' e 'venduto', ma la pratica che ha firmato il contratto e
     aspetta o ha fatto il rogito sta nello stato 'vendita', che non era
     previsto da nessuna riga. Chi era in 'vendita' non faceva scattare
     niente e restava al primo scalino.
     Colpiva soprattutto i VENDITORI: non avendo visite proprie partono da
     zero, quindi senza questa riga restavano su "Contatto" per sempre.
     Aggiunto anche il controllo sulla data di rogito compilata: se c'è,
     la fase è il Rogito qualunque cosa dica lo stato. */
  let pipeIdx=0;
  if(visCli.length) pipeIdx=1;
  if(pratCli.some(p=>['acquisito','trattativa'].includes(p.stato))) pipeIdx=2;
  if(pratCli.some(p=>p.stato==='proposta')) pipeIdx=3;
  if(pratCli.some(p=>p.stato==='vendita'||p.stato==='venduto'||String(p.drogito||p.dataRogito||'').trim())) pipeIdx=4;
