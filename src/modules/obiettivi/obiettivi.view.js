...
var OBJ_COLORI = {
  surplus:{bg:'transparent', bd:OBJ_T.verde, tx:OBJ_T.verde, et:'Surplus'},
  linea:  {bg:'transparent', bd:OBJ_T.ambra, tx:OBJ_T.ambra, et:'In linea'},
  deficit:{bg:'transparent', bd:OBJ_T.rosso, tx:OBJ_T.rosso, et:'Deficit'},
  neutro: {bg:'transparent', bd:'var(--border)', tx:'var(--text3)', et:'Non iniziato'}
};
/* mattoni dello stile comune (index); se mancassero, un ripiego minimo */
function _gxC(et, val, sot, col){
...
function _gxColonna(tit, sot, col, dentro){
  return '<div class="gx-col"><div class="gx-head" style="color:'+col+'"><div class="t">'+tit+'</div>'
    + '<div class="s">'+(sot||'')+'</div></div>' + dentro + '</div>';
}

function _objPillola(f){
  var c = OBJ_COLORI[f] || OBJ_COLORI.neutro;
  return '<span class="gx-bad" style="color:'+c.tx+';border-color:'+c.bd+'">'+c.et+'</span>';
}
...
function _objGrafico(R){
  ...
  return _gxColonna('Andamento dell\'anno', 'cumulato mese per mese', OBJ_T.blu,
...
function _objKpi(R){
  ...
  var colRR = (rr.valore != null && rr.valore >= R.targetAnno) ? OBJ_T.verde : OBJ_T.rosso;
  return '<div class="gx-cards">'
    + _gxC('Obiettivo '+R.anno, _e(R.targetAnno), 'GCI lordo', OBJ_T.blu)
    + _gxC('Realizzato', _e(R.realeAnno), a.perc!=null ? a.perc+'% del dovuto a oggi' : 'nessun obiettivo fissato',
           OBJ_COLORI[a.fascia].tx)
    + _gxC('Dovuto a oggi', _e(R.targetMaturato), 'quota di obiettivo maturata', OBJ_T.ardesia)
    + _gxC('Proiezione dicembre', rr.valore!=null ? _e(rr.valore) : '—',
           rr.attendibile ? 'al ritmo tenuto finora' : 'troppo presto per essere attendibile',
           rr.attendibile ? colRR : OBJ_T.ardesia)
    + '</div>';
}
...
function _objLinguette(){
  ...
  var col = OBJ_T.blu;
  return '<div class="gx-ling">'
    + v.map(function(x){
        var on = (_objTab===x[0]);
        return '<button class="gx-l'+(on?' on':'')+'" style="'+(on?'background:'+col:'')+'" '
          + 'onclick="_objVaiTab(\''+x[0]+'\')"><span class="pall" style="background:'+col+'"></span>'+x[1]+'</button>';
      }).join('') + '</div>';
}

function _objTabella(R){
  var righe = R.livelli[_objTab] || [];
  var nomi = {mensile:'Mese per mese', trimestrale:'Trimestre per trimestre', semestrale:'Semestre per semestre', annuale:'Anno intero'};
  var h = '<div class="gx-col"><div class="gx-head" style="color:'+OBJ_T.blu+'"><div class="t">Scostamenti · '
    + (nomi[_objTab]||'') + '</div><div class="s">obiettivo, dovuto a oggi e realizzato</div></div>'
    + '<div style="overflow:auto">'
    + '<table style="width:100%;border-collapse:collapse;font-size:0.86rem;font-variant-numeric:tabular-nums">'
    + '<thead><tr>'
    + ['Periodo','Obiettivo','Dovuto a oggi','Realizzato','Scostamento','']
        .map(function(t,i){
          return '<th style="padding:9px 12px;text-align:'+(i===0?'left':(i===5?'center':'right'))
            + ';font-size:0.68rem;text-transform:uppercase;letter-spacing:.4px;color:var(--text3);'
            + 'border-bottom:1px solid var(--border);white-space:nowrap">'+t+'</th>';
        }).join('') + '</tr></thead><tbody>';
  righe.forEach(function(r, ir){
    ...
    h += '<tr style="border-bottom:1px solid var(--border);background:'+(ir%2?'rgba(127,127,127,.04)':'transparent')+'">'
  ...
  if(_objTab === 'mensile'){
    ...
    h += '<div class="gx-nota" style="margin:-6px 0 14px">'
      + (_conPesi
...
function _objPipeline(R){
  ...
  return '<div class="gx-col"><div class="gx-head" style="color:'+colV+'"><div class="t">Ce la faccio con quello che ho</div>'
    + '<div class="s">incassato, rogitato, proposte e trattative</div></div><div style="padding:12px 16px">'
    + verdetto
...
function _objConfronto(R){
  ...
  if(p.deltaPerc === null){
    ...
    segno = p.delta >= 0 ? '+' : '';
    col = p.delta >= 0 ? OBJ_T.verde : OBJ_T.rosso;
  ...
  return '<div class="gx-info">' + testo + '</div>';
}

function _objPasso(n, tit, sot, col){
  return _gxC(tit, n, sot, col || OBJ_T.blu);
}
...
function _objResiduo(R){
  ...
  if(r.raggiunto){
    return _gxColonna('Obiettivo raggiunto', '', OBJ_T.verde,
      '<div style="padding:12px 16px;font-size:0.88rem;color:var(--text2)">Hai superato l\'obiettivo di <b style="color:'+OBJ_T.verde+'">'
      + _e(r.oltre) + '</b>' + (r.giorni > 0 ? ', con ' + r.giorni + ' giorni ancora davanti' : '') + '.</div>');
  }
  if(!r.annoInCorso){
    return '<div class="gx-info">Mancavano <b>' + _e(r.manca) + '</b> all\'obiettivo di quell\'anno.</div>';
  }

  var testaRitmo = '';
  if(r.ritmoServe != null){
    var m = r.moltiplicatore;
    var colM = (m == null) ? 'var(--text2)' : (m <= 1 ? OBJ_T.verde : (m <= 1.5 ? OBJ_T.ambra : OBJ_T.rosso));
  ...
  var riquadri = (r.vendite == null)
    ...
    : '<div class="gx-cards" style="margin:12px 0 0">'
  ...
  return _gxColonna('Da qui a fine anno', 'quanto manca, tradotto in ritmo e in affari', OBJ_T.ambra,
      '<div style="padding:12px 16px">'
    + '<div style="font-size:1.5rem;font-weight:800;font-variant-numeric:tabular-nums">' + _e(r.manca)
...
function _objFunnel(R){
  ...
  if(f.venditeServono == null){
    return '<div class="gx-info">Indica la provvigione media prevista '
      + 'nella configurazione per vedere quante vendite, incarichi e valutazioni servono.</div>';
  }
  return '<div class="gx-cards" style="margin-bottom:6px">'
    ...
    + _objPasso(f.venditeFatte, 'Fatte finora', 'provvigioni conteggiate', OBJ_T.verde)
    + '</div>'
    + '<div class="gx-nota" style="margin:0">'
...
function _objWizard(cfg){
  ...
  return '<div id="obj-wizard" class="gx-col"><div class="gx-head" style="color:'+OBJ_T.blu+'">'
    + '<div class="t">Obiettivi '+cfg.anno+'</div><div class="s">quanto vuoi produrre e come lo distribuisci</div></div>'
...
function _objDisegna(){
  ...
  /* [17 set 2026] testata come il Bilancio: titolo con l'anno, linguette dei
     periodi, comandi a destra */
  var _nomeAg = '';
  ...
  var h = '<div class="gx-testa">'
    ...
    +   '<select onchange="_objCambiaAnno(this.value)" style="font:inherit;font-size:1.5rem;font-weight:800;'
    ...
    +   '</select></span>'
    + (senzaObiettivo ? '' : _objLinguette())
    + '<div class="gx-destra">'
    +   (_nomeAg ? '<div class="a">'+_esc(_nomeAg)+'</div>' : '')
    +   '<div class="b" style="display:flex;gap:6px;justify-content:flex-end;flex-wrap:wrap;margin-top:3px">'
    +     '<button class="gx-btn" onclick="_objMostraWizard(true)">'
    +       (senzaObiettivo ? 'Fissa gli obiettivi' : 'Modifica obiettivi') + '</button>'
    +     '<button class="gx-btn" onclick="_objDisegnaPub()">Ricalcola</button>'
    +   '</div>'
    + '</div></div>';
  ...
  if(senzaObiettivo && !_objWizardAperto){
    h += '<div class="gx-col" style="padding:22px;text-align:center">'
    ...
    h += _objConfronto(R) + _objKpi(R) + _objPipeline(R) + _objGrafico(R) + _objTabella(R) + _objScarti(R)
      + _objResiduo(R)
      + _gxColonna('Sull\'anno intero', 'quanti affari servono in dodici mesi', OBJ_T.ardesia,
          '<div style="padding:12px 16px">' + _objFunnel(R) + '</div>');
