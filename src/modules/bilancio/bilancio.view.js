    ...
    try{
      ...
      if(G){
        ...
        var somma = function(arr){
          ...
          return {t:_ceArr(t), q:q};
        };
        var acc = somma(G.accettate), cor = somma(G.inCorso);
        var f = R.percAgente/100;
        R.pipeline = {accettate:acc, inCorso:cor, perc:R.percAgente};
        agg('prev', 'ricavi', 'Proposte accettate', _ceArr(acc.t*f),
            acc.q + ' in attesa di rogito · tua quota al ' + R.percAgente + '%');
        agg('prev', 'ricavi', 'Trattative in corso', _ceArr(cor.t*f),
            cor.q + ' proposte aperte · tua quota al ' + R.percAgente + '%');
      }
...
+ '#ce-wrap .ce-riga.mano:hover .ce-cmd{opacity:1}'
...
function _ceColonna(R, titolo, sezioni, vista, conPrev, tinta){
  ...
  sezioni.forEach(function(sez){
    ...
    if(!righe.length){
      h += '<div class="ce-vuoto">Nessuna voce registrata</div>';
    }
...
function _ceFormDiv(sez){
  ...
  return '<div style="padding:10px 12px;background:var(--bg2);border:1px solid var(--border);border-radius:8px">'
    ...
    + '"Prevista" compare solo nel conto provvisorio; "già registrata" in tutti e due.</div>'
    + _ceElencoVoci(sez)
    + '</div>';
}
...
function ceDisegna(){
  ...
  var h = '<div style="display:flex;align-items:flex-end;gap:14px;flex-wrap:wrap;'
    ...
    + '<div style="display:flex;align-items:center;gap:8px;flex-wrap:nowrap;flex:1;min-width:0">'
    ...
    + '<div style="text-align:right;line-height:1.4;min-width:0;margin-left:auto">'
