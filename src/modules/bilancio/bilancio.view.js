...
function ceCalcola(anno){
  ...
  (PRO === 'titolare' ? [] : prov).forEach(function(p){
    ...
    if(netto > 0 && lordo > 0){
      ...
      var _tc = (spetta > 0) ? _ceArr(t1 * ricevuto / spetta) : 0;
      trattCassa += _tc;
      if(_tc > 0) CD.trattenute.push({et:etich, imp:_tc, data:p.data,
        nota:'sulla quota incassata'});
    }
    quoteCassa += ricevuto;
    if(ricevuto > 0){
      var _mods = (Array.isArray(p.modAgenteRighe) ? p.modAgenteRighe : [])
        .map(function(r){ return r && r.mod; }).filter(function(x){ return x; });
      CD.quote.push({et:etich, imp:ricevuto, data:p.data,
        nota:(_mods.length ? _mods.join(', ') : '') + (ricevuto < spetta ? (_mods.length ? ' · ' : '') + 'acconto' : '')});
    }
    restiTot += resta;
...
function _ceDocCassa(R){
  ...
  var righeE = tit
    ...
    : [['Quote agente incassate', C.quoteRicevute, R.quanteProv + ' provvigioni maturate nel ' + R.anno, 'quote'],
       ['Altre entrate incassate', C.entrateIncassate, '', 'altreEntrate']];
  var righeU = tit
    ...
    : [['Trattenute ufficio', C.trattenute, 'scalate dalle quote incassate', 'trattenute'],
       ['Spese di struttura pagate', C.pagatoStruttura, '', 'struttura'],
       ['F24 versati', C.pagatoImposte, '', 'imposte']];
...
function _ceStampaCassa(R, conDettaglio){
  ...
  var h = '<table class="out"><tr>'
    + col('Entrato', (R.prospettiva === 'titolare'
                      ...
                      : [['Quote agente incassate', C.quoteRicevute, 'quote']])
  ...
  h += '<div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;'
    ...
    + (_tit
      ...
      : 'Volume generato per l\'agenzia nel ' + R.anno + ': <b style="color:var(--text)">' + _ceE(R.gci)
        ...
        + '. Il cruscotto Obiettivi misura il primo numero, questa pagina il secondo.')
    + '</div>';
