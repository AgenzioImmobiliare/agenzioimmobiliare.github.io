...
function mobSaveVisita(){
  ...
  if(_mobEditingVisitaIdx !== null){
    /* In modifica: conserva l'id della visita originale */
  ...
  /* PUSH IMMEDIATO e FORZATO a Firestore (non debounced): garantisce che la
     visita salga al cloud SUBITO, prima che l'app possa essere chiusa o il
     telefono vada in standby. Il push debounced precedente era ritardato e
     poteva non partire, causando la perdita della visita. */
  ...
  /* Fallback: se il push forzato non è disponibile, usa il debounced */
  if(!_pushFatto){
  ...
  /* Conferma: distingue se la visita è stata anche spinta al cloud */
...
function mobDelVisita(i){
  ...
  var _v = D.visite[i];
  /* Blocklist + push forzato, come nel delVisita desktop, così la visita
     eliminata non viene re-importata dal cloud. */
