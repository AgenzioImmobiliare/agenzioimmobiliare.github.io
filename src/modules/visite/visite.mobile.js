...
/* [18 set 2026] CAMBIO DI PERSONA SULLA VISITA.
   Chi fissa una visita senza sapere chi verrà scrive un nome provvisorio —
   spesso quello del proprietario — e sceglie quel nominativo in rubrica.
   Quando poi il nome viene corretto con la persona che è venuta davvero, il
   collegamento restava sul vecchio: la visita finiva nella scheda di un
   cliente che non c'entra, e sulla scheda del telefono compariva ancora quel
   nome.
   Regola: si toglie il collegamento solo quando PRIMA il nome scritto era
   quello del nominativo collegato e ORA è un'altra persona. Se in questa
   modifica hai scelto un nominativo diverso dalla tendina, comanda la tua
   scelta e non si tocca niente. */
function _mobPariNome(a, b){
  return String(a||'').trim().toLowerCase().replace(/\s+/g, ' ')
       === String(b||'').trim().toLowerCase().replace(/\s+/g, ' ');
}
function _mobNomeCollegato(rec){
  if(!rec || typeof D === 'undefined' || !Array.isArray(D.clienti)) return '';
  var c = null;
  if(rec.cliUuid) c = D.clienti.find(function(z){ return z && z.uuid === rec.cliUuid; });
  if(!c){ var r = parseInt(rec.cliRef); if(!isNaN(r) && D.clienti[r]) c = D.clienti[r]; }
  return (c && c.nome) ? String(c.nome) : '';
}
function _mobSganciaSeCambiaPersona(vecchio, nuovo){
  var prima = _mobNomeCollegato(vecchio);
  if(!prima) return false;
  if(!_mobPariNome(prima, _mobNomeCollegato(nuovo))) return false;   /* hai scelto un altro nominativo */
  if(!_mobPariNome(vecchio && vecchio.cliente, prima)) return false; /* il nome scritto non era quello */
  var ora = String((nuovo && nuovo.cliente) || '').trim();
  if(!ora || _mobPariNome(ora, prima)) return false;                 /* il nome non è cambiato */
  nuovo.cliRef = '';
  nuovo.cliUuid = '';
  try{ if(typeof mobToast === 'function') mobToast('Nome cambiato: ho tolto il collegamento in rubrica a ' + prima); }catch(e){}
  return true;
}
try{ window._mobSganciaSeCambiaPersona = _mobSganciaSeCambiaPersona; }catch(e){}
...
function mobSaveVisita(){
  ...
  if(_mobEditingVisitaIdx !== null){
    ...
    try{ _mobSganciaSeCambiaPersona(_old, _fuso); }catch(_e){}
