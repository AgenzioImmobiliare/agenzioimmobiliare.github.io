...
function mobSaveVisita(){
  ...
  /* ── Agente che effettua la visita (firma automatica per terminale) ──
     Principio: la visita registrata da un terminale porta il nome di chi è
     loggato su quel terminale (come fa già il PC in saveVisita).
     Priorità:
       1. MODIFICA di una visita che ha già un agente → lo preservo (non
          sovrascrivo l'assegnazione fatta dal PC/altro agente).
       2. CREAZIONE → firmo con l'utente loggato (_currentUser), come il PC.
       3. Ripiego: se non c'è utente loggato, eredito dall'immobile.
       4. Altrimenti resta vuoto.
     Salvo i due campi nella stessa forma del PC: agenteRef (indice in
     D.agenti, se trovato per nome) + agente (nome testo). */
  var _vecchiaV = null;
  if(_mobEditingVisitaId){
    var _viOld = (D.visite||[]).findIndex(function(x){ return x && x.id === _mobEditingVisitaId; });
    if(_viOld >= 0) _vecchiaV = D.visite[_viOld];
  }
  /* Cerca l'indice in D.agenti corrispondente a un nome (per allineare agenteRef) */
  var _agRefDaNome = function(nome){
    if(!nome || !Array.isArray(D.agenti)) return '';
    var n = String(nome).trim().toLowerCase();
    var ix = D.agenti.findIndex(function(a){ return a && String(a.nome||'').trim().toLowerCase() === n; });
    return (ix >= 0) ? ix : '';
  };
  if(_vecchiaV && _vecchiaV.agente){
    /* 1. Modifica: preservo l'agente già assegnato (incluso uuid stabile) */
    v.agenteRef = (_vecchiaV.agenteRef!==undefined) ? _vecchiaV.agenteRef : '';
    v.agente = _vecchiaV.agente;
    if(_vecchiaV.agenteUuid) v.agenteUuid = _vecchiaV.agenteUuid;
  } else if((typeof _agenteCorrente==='function') && _agenteCorrente()){
    /* 2a. Creazione: firma con l'agente collegato all'utente loggato
       (collegamento stabile usrRef → uuid). Preferito. */
    var _agC = _agenteCorrente();
    v.agente = _agC.nome; v.agenteRef = _agC.ref; v.agenteUuid = _agC.uuid;
  } else if(typeof _currentUser!=='undefined' && _currentUser && (_currentUser.nome || _currentUser.user)){
    /* 2b. Ripiego: utente loggato non collegato a un agente → firma col nome */
    v.agente = _currentUser.nome || _currentUser.user || '';
    v.agenteRef = _agRefDaNome(v.agente);
  } else if(im && im.agenteRef!==undefined && im.agenteRef!==null && im.agenteRef!==''){
    /* 3. Ripiego: nessun utente loggato → eredito dall'immobile */
    var _agIxM = parseInt(im.agenteRef);
    if(!isNaN(_agIxM) && D.agenti && D.agenti[_agIxM]){
      v.agenteRef = _agIxM;
      v.agente = (D.agenti[_agIxM].nome) || '';
    } else { v.agenteRef=''; v.agente=''; }
  } else { v.agenteRef=''; v.agente=''; }
  if(_mobEditingVisitaIdx !== null){
    try{ var _oldVm = D.visite[_mobEditingVisitaIdx]; if(_oldVm && _oldVm.id) v.id = _oldVm.id; }catch(_e){}
    D.visite[_mobEditingVisitaIdx] = v;
  } else if(_mobEditingVisitaId){
    /* Rete di sicurezza anti-duplicato: stavamo modificando una visita
       (avevamo un id) ma l'indice si è perso. NON creare un doppione:
       ri-cerca per id e aggiorna quella esistente. Crea una nuova solo
       se davvero non esiste più. */
    var _idxById = (D.visite||[]).findIndex(function(x){ return x && x.id === _mobEditingVisitaId; });
    if(_idxById >= 0){ v.id = _mobEditingVisitaId; D.visite[_idxById] = v; }
    else { D.visite.push(v); }
  } else {
    D.visite.push(v);
  }
  ...
  mobToast(_mobEditingVisitaIdx !== null ? 'Visita aggiornata' : (_pushFatto ? '✓ Visita salvata e sincronizzata' : 'Visita salvata (sync in corso…)'));
  _mobEditingVisitaIdx = null; _mobEditingVisitaId = null;
}
