...
function mobSaveVisita(){
  ...
  /* ── Agente che effettua la visita ──
     - In MODIFICA di una visita che ha già un agente: lo preservo (non lo
       sovrascrivo, altrimenti un salvataggio da mobile cancellerebbe
       l'assegnazione fatta dal PC).
     - In CREAZIONE (o se la visita non aveva agente): eredito dall'immobile
       (im.agenteRef); se l'immobile non ha agente, resta vuoto.
     Salvo i due campi nella stessa forma del PC: agenteRef (indice) + agente (nome). */
  var _vecchiaV = null;
  if(_mobEditingVisitaId){
