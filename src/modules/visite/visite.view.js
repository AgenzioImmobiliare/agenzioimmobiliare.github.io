...
function delVisita(id){
  ...
  if(confirm('Eliminare questa visita'+(v&&v.cliente?' di '+v.cliente:'')+'?')){
    D.visite.splice(i,1);
    saveD();
    /* Forza il push immediato al cloud (non aspetta il ritardo normale di
       ~1-2 secondi): una cancellazione è un'operazione importante da non
       perdere se chiudi la scheda subito dopo. */
    try{ if(typeof window._forcePushNow==='function') window._forcePushNow(); }catch(_e){}
    refreshCurrentView();
  }
}
