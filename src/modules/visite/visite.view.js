...
function saveVisita(){
  try{
    ...
    var _esitoDiventatoNegativo = false; /* per estendere il flusso "crea richiesta" alle MODIFICHE */
    if(D.editIdx!==null&&D.editType==='visita'){
      ...
      try{
        ...
        /* L'esito DIVENTA negativo ora (non lo era già prima): proporremo la richiesta */
        var _oldNeg = (_oldEs.toUpperCase()==='RIFIUTATO' || _oldEs.toUpperCase()==='SCONOSCIUTO');
        var _newNeg = (_newEs.toUpperCase()==='RIFIUTATO' || _newEs.toUpperCase()==='SCONOSCIUTO');
        if(_newNeg && !_oldNeg) _esitoDiventatoNegativo = true;
      ...
      /* Conserva l'id della visita originale (la modifica non deve cambiarne
         l'identità, altrimenti il sync può trattarla come nuova → duplicato).
         Se l'originale non aveva id (visita legacy), tiene quello nuovo. */
      try{ if(_oldV && _oldV.id) v.id=_oldV.id; }catch(_e){}
    } else {
      D.visite.push(v);
    ...
    if((isNewVisita || _esitoDiventatoNegativo) && _esitoNegativo && (v.cliente||'').trim()){
      var _vCopy = v;
...
function delVisita(i){
  ...
  if(confirm('Eliminare questa visita?')){
    var _v = D.visite[i];
    /* Registra l'id nella blocklist così il sync cloud non la re-importa
       (causa delle "visite che ricompaiono"). */
    try{ if(_v && _v.id && typeof window._visAddDeletedId==='function') window._visAddDeletedId(_v.id); }catch(_e){}
    D.visite.splice(i,1);
    saveD();
    /* Push immediato della cancellazione su Firestore */
...
function renderVisite(){
  ...
  tbody.innerHTML=f.length?f.map(({v,origIdx},i)=>{
    ...
    const foto=im?.foto||'';
    const fotoCell=foto
      ?`<td style="padding:6px 8px"><img src="${foto}" style="width:96px;height:72px;object-fit:cover;border-radius:9px;border:1px solid var(--border);display:block;cursor:pointer" onclick="openSchedaImmobile(${immIdx})" loading="lazy" title="${im?.tipo||''} — ${im?.comune||''}"></td>`
      :`<td style="padding:6px 8px"><div style="width:96px;height:72px;background:#F1F5F9;border-radius:9px;border:1px solid var(--border);display:flex;align-items:center;justify-content:center;color:#CBD5E1"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg></div></td>`;
    return`<tr>${fotoCell}<td style="color:var(--text3)">${i+1}</td><td style="font-weight:600;white-space:nowrap">${fmtD(v.data)}</td><td>${v.ora||'—'}</td><td style="cursor:pointer;color:var(--brand)" onclick="${im?`openSchedaImmobile(${immIdx})`:'void(0)'}">${v.immTitolo||v.ref||'—'}</td><td style="font-weight:600">${v.cliente||'—'}</td><td>${v.tel?`<a href="tel:${v.tel}" style="color:var(--brand)">${v.tel}</a>`:'—'}</td><td style="font-size:0.8rem;font-weight:600;color:var(--text2)">${v.agente||(v.agenteRef!==undefined&&v.agenteRef!==''&&D.agenti&&D.agenti[v.agenteRef]?D.agenti[v.agenteRef].nome:'')||'<span style=\'color:var(--red-l)\'>— da assegnare</span>'}</td><td style="font-size:0.8rem;color:var(--text3)">${v.agenzia||'—'}</td><td>${bEsito(v.esito)}</td><td><span class="badge badge-gray" style="font-size:0.68rem">${v.feedback||'—'}</span></td><td class="note-cell">${v.note||'—'}</td><td><div class="actions-col"><button class="icon-btn" onclick="editVisita(${ri})" title="Modifica"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button><button class="icon-btn" onclick="delVisita(${ri})" style="color:var(--red-l)" title="Elimina"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg></button></div></td></tr>`;
