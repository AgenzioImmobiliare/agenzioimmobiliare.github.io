...
function delVisita(id){
  ...
  if(confirm('Eliminare questa visita'+(v&&v.cliente?' di '+v.cliente:'')+'?')){
    ...
    /* [22 set 2026] Se la visita è nata da un appuntamento dell'agenda e
       l'appuntamento è ancora lì, cancellare la sola visita non serve: al primo
       salvataggio dell'agenda rinasce, con un id nuovo che nessuna lista di
       cancellati riconosce. Quindi chiedo cosa fare dell'appuntamento. */
    try{
      var _ev = null, _evI = -1;
      if(v && v._daEventoId && Array.isArray(D.eventi)){
        for(var _q = 0; _q < D.eventi.length; _q++){
          if(D.eventi[_q] && D.eventi[_q].id === v._daEventoId){ _ev = D.eventi[_q]; _evI = _q; break; }
        }
      }
      if(_ev){
        var _qd = (function(d){ var p = String(d||'').split('-'); return p.length===3 ? (p[2]+'/'+p[1]+'/'+p[0]) : String(d||''); })(_ev.data);
        var _msg = 'Questa visita nasce dall\'appuntamento in agenda del ' + _qd + (_ev.ora ? (' alle ' + _ev.ora) : '') + '.\n\n'
          + 'OK = cancello anche l\'appuntamento.\n'
          + 'Annulla = tengo l\'appuntamento, ma non ricreerà più questa visita.';
        if(window.confirm(_msg)){
          D.eventi.splice(_evI, 1);
          if(typeof showToast==='function') showToast('Visita e appuntamento eliminati','','#15803D');
        } else {
          _ev._noVisita = true;
          if(typeof showToast==='function') showToast('Visita eliminata · l\'appuntamento resta in agenda','','#15803D');
        }
      }
    }catch(_e){ console.warn('[Visite] appuntamento collegato:', _e); }
    saveD();
