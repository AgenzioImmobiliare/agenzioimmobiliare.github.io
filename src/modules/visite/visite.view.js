...
function saveVisita(){
  try{
    ...
    if(_agRefRaw!==''&&_agRefRaw!==null&&!isNaN(parseInt(_agRefRaw))){
      ...
      if(D.agenti&&D.agenti[_agIx]&&D.agenti[_agIx].uuid) v.agenteUuid=D.agenti[_agIx].uuid;
    } else {
      /* Nessun agente scelto: firma automatica con l'agente collegato
         all'utente loggato (collegamento stabile usrRef → uuid). */
      var _agC=(typeof _agenteCorrente==='function')?_agenteCorrente():null;
      if(_agC){
        v.agente=_agC.nome; v.agenteRef=_agC.ref; v.agenteUuid=_agC.uuid;
      } else {
        v.agenteRef=''; v.agente='';
      }
    }
...
function renderVisite(){
  ...
  sorted=sorted.filter(({v})=>{
    ...
    if(!((!q||t.includes(q))&&(!esito||(v.esito||'').toUpperCase()===esito)&&(immF===''||String(v.immRef)===String(immF)))) return false;
    /* Filtro "solo le mie visite": confronta con l'agente collegato all'utente
       loggato. Usa l'uuid stabile dove c'è (visite nuove), ripiega sul nome per
       le visite vecchie che non hanno agenteUuid. */
    var _soloMie=document.getElementById('f-vis-solomie');
    if(_soloMie && _soloMie.checked){
      var _agC=(typeof _agenteCorrente==='function')?_agenteCorrente():null;
      if(_agC){
        var _match = (v.agenteUuid && _agC.uuid && v.agenteUuid===_agC.uuid)
          || (!v.agenteUuid && (v.agente||'').trim().toLowerCase()===(_agC.nome||'').trim().toLowerCase());
...
function pulisciDoppioniVisite(){
  ...
  var nrmOra = function(o){ return String(o==null?'':o).trim().replace('.',':').slice(0,5); };
  var nrmData = function(d){ return String(d==null?'':d).trim().slice(0,10); };
  var telOf = function(v){ return nrm(v.tel).replace(/[^0-9]/g,''); };
  /* Chiave d'identità di una visita: immobile+data+ora+(cliente o telefono).
     Ora e data normalizzate così "16.00"/"16:00" e date con orario coincidono. */
  var chiave = function(v){
    var cli = nrm(v.cliente) || telOf(v);
    return [String(v.immRef), nrmData(v.data), nrmOra(v.ora), cli].join('|');
  };
  /* "Punteggio" per decidere quale tenere: più alto = più completa.
     Ha esito diverso da IN ATTESA/DA-FARE → +2 ; ha agente → +1 ; ha feedback → +1 */
  var punteggio = function(v){
    var p = 0;
    var es = nrm(v.esito);
    if(es && es !== 'in attesa' && es !== 'da-fare') p += 2;
  ...
  /* Raccogli gli ID dei doppioni PRIMA di rimuoverli (lo splice cambia gli indici).
     IMPORTANTE: registro nella blocklist SOLO per id specifico del doppione, MAI
     per chiave composita — perché la copia buona ha la stessa data/ora/cliente/
     immobile e verrebbe bloccata anche lei. Se un doppione non ha id, non lo metto
     in blocklist (lo bloccherei insieme alla buona): mi affido al merge che ora
     tiene la versione più completa. */
  var _idsDaBloccare = [];
  daRimuovere.forEach(function(idx){
    var _v = D.visite[idx];
    if(_v && _v.id) _idsDaBloccare.push(_v.id);
  });
  ...
  /* Registra gli id rimossi nella blocklist così il merge col cloud non li
     re-inserisce al prossimo reload (usa l'id, non la chiave composita). */
  if(typeof _visAddDeletedId === 'function'){
    _idsDaBloccare.forEach(function(id){ _visAddDeletedId(id); });
  }

  saveD();
  /* Push immediato al cloud: rende la rimozione permanente subito, senza
     aspettare il debounce. Senza questo, un reload rapido ripescava i doppioni. */
  if(typeof _visForcePushNow === 'function'){ try{ _visForcePushNow(); }catch(e){} }

  if(typeof renderVisite==='function') renderVisite();
  if(typeof updateBadges==='function') try{ updateBadges(); }catch(e){}
  var _senzaId = daRimuovere.length - _idsDaBloccare.length;
  var _msgOk = daRimuovere.length+' doppioni rimossi';
  if(_senzaId > 0) _msgOk += ' ('+_senzaId+' senza id: se ricompaiono, ricarica e rilancia)';
  if(typeof showToast==='function') showToast(_msgOk,'','#15803D');
  else alert(_msgOk);
}
