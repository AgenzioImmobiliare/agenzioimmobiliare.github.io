...
function _visWaAttesa(v){
  try{
    ...
    var n = String(v.tel).replace(/\D/g,'');
    if(n.indexOf('39')===0){} else if(n.length===10||n.length===9){ n='39'+n; }
    if(!n) return '';
    var nome = v.cliente ? String(v.cliente).split(' ')[0] : '';
    var msg = 'Salve'+(nome?' '+nome:'')+', come è andata la visita? Mi faccia sapere se l\'immobile è di suo interesse. Grazie!';
