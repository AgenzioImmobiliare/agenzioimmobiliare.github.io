...
/* [17 set 2026] Posizione della provvigione che ha generato un incasso NF:
   prima la targa (_provUuid), il "Prov#N" della descrizione solo per le
   scritture senza targa. Il Prov#N è la posizione di quando la scrittura è
   nata e scivola quando si cancella una provvigione: così l'acconto del
   Ref.0038 finiva sotto il Ref.0001. */
function _partProvIdxDi(nf){
  var PR = D.provvigioni || [];
  if(nf && nf._provUuid){
    for(var k = 0; k < PR.length; k++){ if(PR[k] && PR[k].uuid === nf._provUuid) return k; }
    return -1;
  }
  var m = String((nf && nf.descr) || '').match(/Prov#(\d+)/);
...
function _partBuildMastroAgenti(){
  ...
  (D.provvigioni||[]).forEach(function(pv, pi){
    ...
    /* AVERE — ABBUONO (quota rinunciata dall'agente).
       [25 ago 2026] Mancava del tutto. Nella scheda provvigione l'abbuono
       chiude il conto (residuo 0), mentre qui il partitario non lo conosceva e
       restava aperto per quella cifra: su MAZZA ANNA maturato 5.675,00 contro
       5.674,88 fra trattenuta e pagamenti, con 0,12 di residuo fantasma.
       Non è un pagamento ma una rinuncia, e come le trattenute va in AVERE:
       riduce quello che l'agenzia deve ancora dare. Compare come riga a sé
       così si vede anche il perché il conto si chiude. */
    var abbuonoAgt = parseFloat(pv.abbuono)||0;
    if(abbuonoAgt>0.001){
      mastro[agNome].partite.push({
        id:'prov_abb_'+pi, tipo:'avere', ndoc:ndoc,
        data:dataProv, descrizione:'Abbuono (quota rinunciata) — '+descBase,
        importo:abbuonoAgt, fonte:'provvigione', fonteIdx:pi,
        stato:'chiusa', modalita:'Abbuono'
      });
      mastro[agNome].totAvere += abbuonoAgt;
      avereDellaProv += abbuonoAgt;
    }
