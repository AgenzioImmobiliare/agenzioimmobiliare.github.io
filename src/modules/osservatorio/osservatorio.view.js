...
(function(){
  ...
  /* [17 set 2026] toni del Bilancio (verde bosco, blu notte, ambra, ardesia,
     rosso scuro, viola), gli stessi dello stile comune gx */
  var TINTE = {verde:'#0F5132', blu:'#1E3A8A', ambra:'#B45309', ardesia:'#334155', rosso:'#7F1D1D', viola:'#5B21B6'};
  ...
  function kpiCard(label, value, sub, color){
    /* [17 set 2026] riquadro dello stile comune */
    return '<div class="gx-card" style="color:'+(color||TINTE.ardesia)+'">'
      + '<div class="et">'+label+'</div>'
      + '<div class="val" style="color:'+(color||TINTE.ardesia)+'">'+value+'</div>'
      + '<div class="sot">'+(sub||'')+'</div></div>';
  }
  function renderKPI(recs){
    ...
    var html=
      kpiCard('Annunci attivi', fmtN(n), prem+' premium', TINTE.blu)+
      kpiCard('Visite totali', fmtN(visite), 'media '+fmtN(n? visite/n:0)+'/annuncio', TINTE.verde)+
      kpiCard('Contatti', fmtN(contatti), 'conversione '+conv.toFixed(2)+'%', TINTE.ambra)+
      kpiCard('Salvati', fmtN(salvati), 'media '+fmtN(n? salvati/n:0)+'/annuncio', TINTE.viola)+
      kpiCard('Qualità media', qMed+'%', qMed<60?'da migliorare':'buona', qMed<60?TINTE.rosso:TINTE.verde)+
      kpiCard('Quota Premium', (n? Math.round(prem/n*100):0)+'%', prem+' su '+n+' annunci', TINTE.ardesia);
    document.getElementById('imio-kpi').innerHTML=html;
  }

  /* ---- Grafici ---- */
  var PALETTE=['#1E3A8A','#0F5132','#B45309','#5B21B6','#334155','#7F1D1D','#0E7490','#9D174D','#3F6212','#6D28D9'];
  ...
  function renderCharts(recs){
    ...
    charts.tipo=new Chart(document.getElementById('imio-ch-tipo'),{
      ...
      data:{labels:t.map(function(x){return x[0];}),
        datasets:[{data:t.map(function(x){return x[1];}),backgroundColor:TINTE.blu,borderRadius:4}]},
    ...
    charts.comune=new Chart(document.getElementById('imio-ch-comune'),{
      ...
      data:{labels:c.map(function(x){return x[0];}),
        datasets:[{data:c.map(function(x){return x[1];}),backgroundColor:TINTE.verde,borderRadius:4}]},
    ...
    charts.premium=new Chart(document.getElementById('imio-ch-premium'),{
      type:'doughnut',
      data:{labels:['Premium','Standard'],datasets:[{data:[prem,std],backgroundColor:[TINTE.ambra,'#CBD5E1'],borderWidth:0}]},
    ...
    charts.scatter=new Chart(document.getElementById('imio-ch-scatter'),{
      ...
      data:{datasets:[{
        ...
        backgroundColor:recs.map(function(r){return r.premium?'rgba(180,83,9,.75)':'rgba(30,58,138,.6)';}),
  ...
  var COLS=[
    ...
    {k:'premium', t:'Premium',   align:'center',fmt:function(r){return r.premium?'<span style="color:'+TINTE.ambra+';font-weight:800">★</span>':'';}, val:function(r){return r.premium?1:0;}},
    {k:'gest',    t:'Gestionale',align:'center',
      fmt:function(r){
        var m=r.match||{stato:'non-gest'};
        var cod=esc(r.codice).replace(/'/g,"\\'");
        var inner;
        if(m.stato==='attivo')     inner='<span class="gx-bad" title="Ref. '+esc(m.ref)+' · '+esc(m.how)+'" style="color:'+TINTE.verde+'">● '+(m.ref?esc(m.ref):'attivo')+(m.how==='manuale'?' &#128279;':'')+'</span>';
        else if(m.stato==='non-attivo') inner='<span class="gx-bad" title="Stato: '+esc(m.statoGest)+'" style="color:'+TINTE.ambra+'">● '+esc(m.statoGest||'non attivo')+'</span>';
        else inner='<span class="gx-bad" style="color:var(--text3)">non in gestionale</span>';
  ...
  function fmtDelta(n, invert){
    ...
    var col = good? TINTE.verde : TINTE.rosso;
    var arr = pos? '▲':'▼';
  ...
  window.imioRenderCompare=function(){
    ...
    if(snaps.length<2){
      ...
      if(snaps.length===1){
        ...
        info='<div class="gx-info" style="margin-bottom:10px">&#10003; Import registrato del <strong>'+d1.toLocaleDateString('it-IT',{day:'2-digit',month:'long',year:'numeric'})+'</strong> ('+snaps[0].recs.length+' annunci). Questo &egrave; il tuo primo snapshot storico.</div>';
      }
    ...
    /* [17 set 2026] riquadri dello stile comune */
    var cartella=function(et, val, sot){
      return '<div class="gx-card" style="color:'+TINTE.blu+'"><div class="et">'+et+'</div>'
        + '<div class="val">'+val+'</div><div class="sot">'+(sot||'')+'</div></div>';
    };
    var header='<div class="gx-cards" style="margin-bottom:12px">'
      + cartella('Visite nel periodo', fmtDelta(tot.visiteB-tot.visiteA), 'su '+giorni+' giorni')
      + cartella('Contatti nel periodo', fmtDelta(tot.contattiB-tot.contattiA), '')
      + cartella('Salvati nel periodo', fmtDelta(tot.salvatiB-tot.salvatiA), '')
      + '</div>';

    var trs=rows.map(function(o){
      var r=o.rec;
      var tag = o.isNew? ' <span class="gx-bad" style="color:'+TINTE.blu+'">nuovo</span>'
              : o.isGone? ' <span class="gx-bad" style="color:'+TINTE.rosso+'">uscito</span>' : '';
      return '<tr>'+
  ...
  window.renderImmobiliNonAbbinati=function(){
    ...
    if(!nonAbbinati.length){
      box.innerHTML='<div class="gx-info" style="margin-bottom:0">Tutti gli immobili attivi sono <b style="color:'+TINTE.verde+'">abbinati</b> ai dati statistici.</div>';
      return;
    }
    /* [17 set 2026] colonna dello stile comune, titolo in ambra */
    var righe=nonAbbinati.map(function(o){
      var col = o.motivo==='manca codice' ? TINTE.ambra : TINTE.rosso;
      return '<div class="gx-riga">'
        + '<div style="flex:1;min-width:0"><span class="tt">'+esc(o.ref||'—')+'</span> '
        + '<span class="sub">'+esc(o.tipo)+(o.comune?' · '+esc(o.comune):'')+'</span></div>'
        + '<span class="gx-bad" style="color:'+col+'">'+o.motivo+'</span>'
        + '</div>';
    }).join('');
    box.innerHTML='<div class="gx-col" style="margin-bottom:0">'
      + '<div class="gx-head" style="color:'+TINTE.ambra+'"><div class="t">'+nonAbbinati.length+' immobili attivi senza statistiche</div>'
      +   '<div class="s">aggiungi il codice Immobiliare.it nella loro scheda per abbinarli</div></div>'
      + righe
      + '</div>';
  };
  ...
  function renderMatchSummary(stats){
    ...
    function chip(col,bg,label,val){
      /* [17 set 2026] pallino e numero, senza fondo colorato */
      return '<span style="display:inline-flex;align-items:center;gap:6px;color:var(--text2);font-size:.8rem;font-weight:600">'+
        '<span style="width:8px;height:8px;border-radius:50%;background:'+col+'"></span>'+label+': <b style="color:'+col+'">'+val+'</b></span>';
    }
    el.innerHTML=
      '<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">'+
        chip(TINTE.verde,'','Attivi in gestionale',stats.attivo)+
        chip(TINTE.ambra,'','Non più attivi',stats.nonAttivo)+
        chip(TINTE.ardesia,'','Non in gestionale',stats.nonGest)+
        '<span style="flex:1"></span>'+
        '<span style="font-size:.74rem;color:var(--text3)">Abbinati: '+stats.esatti+' per link · '+stats.manuali+' manuali · '+stats.fuzzy+' per somiglianza · '+tot+' annunci totali</span>'+
      '</div>';
  }
  ...
  window.imioBoot=function(){
    try{ if(typeof window.gxCss==='function') window.gxCss(); }catch(e){}
    var data=loadLS();
  ...
  function injectStyle(){
    ...
    /* [17 set 2026] Veste del Bilancio. Le classi gx- vengono dallo stile
       comune dell'index; qui solo le rifiniture di questa sezione, che ha
       grafici e tabelle scritti con stili in linea (da qui gli !important). */
    var S='#sec-immobiliare ';
    var css=
      '.imio-chart-card{background:var(--bg2);border:1px solid var(--border,#E2E8F0);border-radius:10px;padding:14px 16px;box-shadow:none}'+
      '.imio-chart-title{font-weight:800;font-size:1rem;color:'+TINTE.blu+';padding-bottom:6px;border-bottom:2px solid currentColor;display:inline-block}'+
      '.imio-chart-sub{font-size:.76rem;color:var(--text3);margin-top:4px}'+
      S+'.imio-filtri select,'+S+'#imio-search{padding:6px 12px!important;border-radius:20px!important;'+
        'border:1.5px solid var(--border)!important;background:var(--bg)!important;font-size:.8rem!important;'+
        'font-weight:700!important;color:var(--text2)!important;font-family:inherit}'+
      S+'.imio-filtri select:hover,'+S+'#imio-search:focus{border-color:var(--text3)!important;color:var(--text)!important;outline:none}'+
      S+'#imio-cmp-a,'+S+'#imio-cmp-b{border-radius:20px!important;border:1.5px solid var(--border)!important;background:var(--bg)!important;font-size:.8rem!important}'+
      S+'table th{background:transparent!important;color:var(--text3)!important;font-size:.68rem!important;'+
        'text-transform:uppercase;letter-spacing:.04em;border-bottom:2px solid var(--text)!important}'+
      S+'table th:hover{color:var(--text)!important}'+
      S+'table td{border-bottom:1px solid var(--border)!important;font-variant-numeric:tabular-nums}'+
      S+'table tbody tr:nth-child(even){background:rgba(127,127,127,.04)}'+
      S+'table tbody tr:hover{background:rgba(127,127,127,.08)}'+
      S+'#imio-cmp-body .gx-cards{margin-bottom:12px}';
    var s=document.createElement('style'); s.id='imio-css'; s.textContent=css; document.head.appendChild(s);
    try{ if(typeof window.gxCss==='function') window.gxCss(); }catch(e){}
  }
  injectStyle();
})();
