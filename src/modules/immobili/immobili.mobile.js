...
function mobOpenSchedaImmobile(idx){
  ...
  /* [3 set 2026] Seconda riga di azioni: due comandi che servono quando si è
     FUORI, davanti all'immobile, ed erano solo sulla scheda del computer.
     Riga separata a due colonne perché le etichette sono lunghe e nella
     griglia da quattro sarebbero andate a capo. Le funzioni stanno
     nell'index, quindi ci sono anche qui: mancava solo il pulsante.
     L'ancoraggio è l'uuid, non la posizione nell'elenco. */
  html += '<div class="mi-qa-row" style="grid-template-columns:repeat(2,1fr);padding-top:0">';
  html += '<button class="mi-qa light" onclick="if(typeof preparaIncarico===\'function\')preparaIncarico(\''+E(im.uuid||'')+'\')">'
        + '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h7"/><polyline points="14 2 14 8 20 8"/><path d="M18.4 14.6a2 2 0 013 3L18 21l-3 .6.6-3z"/></svg>'
        + 'Prepara incarico</button>';
  html += '<button class="mi-qa light" onclick="if(typeof apriMappaCatastale===\'function\')apriMappaCatastale(\''+E(im.uuid||'')+'\')">'
        + '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 4h8v7H3z"/><path d="M11 4h10v5H11z"/><path d="M3 11h6v9H3z"/><path d="M9 13h12v7H9z"/><circle cx="15" cy="16.5" r="1.6" fill="currentColor" stroke="none"/></svg>'
        + 'Mappa catastale</button>';
  html += '</div>';
