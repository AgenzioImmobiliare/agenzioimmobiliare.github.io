...
/* [16 set 2026] A CHI APPARTIENE UN IMMOBILE — la stessa regola del modulo
   Clienti. Prima qui il proprietario si cercava con la POSIZIONE nell'elenco
   (clienteRef) o col nome, mentre archiviaCliente usa _immApparteneCli, che
   guarda prima l'uuid: dopo un riordino dovuto alla sincronizzazione i due
   versi dell'archiviazione potevano parlare di due clienti diversi.
   _immApparteneCli vive nel monolite: se mancasse si torna alla regola
   vecchia invece di rompere l'archiviazione. */
function _immDelCliente(im, ci){
  if(typeof window._immApparteneCli === 'function') return window._immApparteneCli(im, ci);
  if(!im) return false;
  if(im.clienteRef!==undefined&&im.clienteRef!==null&&im.clienteRef!=='') return parseInt(im.clienteRef)===ci;
  const cli=D.clienti[ci];
  return !!(cli && im.contatto && im.contatto===cli.nome);
}
function _immProprietarioIdx(im){
  if(!im || !Array.isArray(D.clienti)) return -1;
  for(let ci=0; ci<D.clienti.length; ci++){
    if(D.clienti[ci] && _immDelCliente(im, ci)) return ci;
  }
  return -1;
}
...
function salvaArchiviazione(){
  ...
  const cliRefIdx=_immProprietarioIdx(im);
  if(cliRefIdx>=0){
    ...
    if(cli && !cli.archiviato){
      ...
      const altriAttivi=D.immobili.filter((other,oi)=>{
        if(!other || oi===_archiviaIdx) return false;
        if(statiChiusi.includes((other.stato||'').toLowerCase())) return false;
        return _immDelCliente(other, cliRefIdx);
      });
