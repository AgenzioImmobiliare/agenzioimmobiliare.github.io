...
function openImmobileModal(idx){
  ...
  if(immAgSel){
    /* ATTENZIONE: il valore dell'opzione DEVE essere la posizione reale in
       D.agenti. Prima si faceva .filter().map((a,i)=>...) e `i` era la
       posizione nella lista FILTRATA: bastava un agente "non attivo" prima
       degli altri perché salvando si assegnasse l'immobile all'agente
       sbagliato. Ora si mappa su tutta la lista e si saltano i non attivi. */
    immAgSel.innerHTML='<option value="">-- Seleziona agente --</option>'+
      D.agenti.map((a,i)=>(a && a.stato!=='non attivo')?
        `<option value="${i}">${a.nome||('Agente '+(i+1))}</option>`:'').join('');
  }
...
function saveImmobile(){
  try{
    ...
    const im={
      ...
      /* dati catastali (sezione 3b) — servono anche al riporto dati
         dall'Analisi Atti AI */
      catComune:g('im-cat-comune'),catSezione:g('im-cat-sezione'),
      catFoglio:g('im-cat-foglio'),catParticella:g('im-cat-particella'),
      catSub:g('im-cat-sub'),catCategoria:g('im-cat-categoria'),
      catClasse:g('im-cat-classe'),catConsistenza:g('im-cat-consistenza'),
      catRendita:g('im-cat-rendita'),catIntestatario:g('im-cat-intestatario'),
      agenteRef,
      /* multi-agente: identità stabile dell'agente, accanto all'indice.
         L'indice può scalare se la lista agenti cambia, l'uuid no. */
      agenteUuid:(function(){ var _n=parseInt(agenteRef);
        return (!isNaN(_n)&&D.agenti&&D.agenti[_n])?(D.agenti[_n].uuid||''):''; })(),
...
function renderImmobili(){
  ...
  (function(){
    ...
    /* il valore è l'UUID dell'agente, non la posizione: così il filtro resta
       corretto anche se la lista agenti cambia */
    (D.agenti||[]).forEach(function(a,i){
      if(a&&a.nome){var o=document.createElement('option');o.value=String(a.uuid||i);o.textContent=a.nome;sel.appendChild(o);}
    });
    sel.value=cur;
  })();
  const f=D.immobili.filter(im=>{
    ...
    /* confronto via risolutore unico: uuid stabile, con ripiego su indice/nome
       per i record vecchi non ancora migrati */
    let agenteOk=true;
    if(agenteFiltro){
      const _ag=(typeof window._agenteDi==='function')?window._agenteDi(im,'agenteRef'):null;
      agenteOk = _ag ? (String(_ag.uuid||_ag.ref)===String(agenteFiltro)) : false;
    }
    ...
    <div style="display:flex;gap:16px;margin-bottom:18px;align-items:flex-start;flex-wrap:wrap">
      ...
      <div style="flex:1;min-width:220px">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:8px">
          ...
          <!-- Pulsanti azione (ridisegnati: gerarchia chiara, 1 primaria + secondarie neutre + menu Altro) -->
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:4px">
            ...
            <!-- SECONDARIE: stile neutro uniforme -->
            <button class="btn simm-btn2" onclick="openImmobileModal(${i})" title="Modifica immobile">${icoEdit} Modifica</button>
            <button class="btn simm-btn2" onclick="apriMappa(${i})" title="Mostra sulla mappa">${icoMap} Mappa</button>
            <button class="btn simm-btn2" onclick="if(typeof reportProprietario==='function')reportProprietario(${i})" title="Report visite e feedback per il proprietario"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> Report</button>
            <button class="btn simm-btn2" onclick="brochureImmobile(${i})" title="Brochure professionale stampabile">${icoPdf} Brochure</button>
            <button class="btn simm-btn2" onclick="showImmQR(${i})" title="QR code del link al portale"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="3" height="3"/><rect x="18" y="18" width="3" height="3"/></svg> QR</button>
            <!-- MENU ALTRO: azioni di stato e distruttive, tolte dalla vista principale -->
            <div style="position:relative;display:inline-block">
              <button class="btn simm-btn2" onclick="var m=document.getElementById('simm-more-${i}');m.style.display=m.style.display==='block'?'none':'block'" title="Altre azioni" style="padding:9px 12px"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="19" cy="12" r="2"/></svg></button>
