...
function immAddCoInt(nome, tel){
  ...
  row.innerHTML=`
    <input type="text" placeholder="Nome e cognome" value="${nome||''}"
      style="flex:1;padding:7px 10px;border:1.5px solid #DDD6FE;border-radius:8px;font-family:inherit;font-size:0.85rem;outline:none;color:var(--text)"
...
function immGetCoInt(){
  ...
  return Array.from(list.children).map(row=>{
    ...
    return {nome:(inputs[0]?.value||'').trim(), tel:(inputs[1]?.value||'').trim()};
  }).filter(x=>x.nome);
}
function immSetCoInt(arr){
  ...
  if(!list) return;
  list.innerHTML='';
  (arr||[]).forEach(x=>immAddCoInt(x.nome,x.tel));
...
function salvaArchiviazione(){
  ...
  const motivoLabel={
    scadenza:'Scadenza incarico non rinnovato',disdetta:'Ripensamento / disdetta cliente',
    venditaprivata:'Vendita privata (senza agenzia)',
  ...
  // ── [PROVVIGIONE] Crea 3 scadenze di verifica, ogni 4 mesi dalla data di
  //    archiviazione. Servono a controllare se l'immobile è stato venduto a un
  //    cliente presentato dall'agenzia durante il mandato → diritto a provvigione.
  (function(){
    // pulizia eventuali verifiche precedenti per questo immobile
    D.eventi=D.eventi.filter(e=>!(e._immIdx===_archiviaIdx && e._type==='verifica_provvigione'));
    const base=new Date((data||today())+'T00:00:00');
    const immDesc=(im.tipo||'Immobile')+(im.comune?' — '+im.comune:'');
    for(let k=1;k<=3;k++){
      const d=new Date(base);
      d.setMonth(d.getMonth()+(4*k)); // +4, +8, +12 mesi
      const ds=d.toISOString().slice(0,10);
      D.eventi.push({
        titolo:`Verifica provvigione (${k}/3): ${immDesc}`,
        tipo:'scadenza', data:ds, ora:'',
        cliente:im.contatto||'', tel:im.telefono||'',
        note:`Controllo post-mandato (${motivoLabel}). Verificare se l'immobile è stato venduto a un cliente presentato dall'agenzia mentre il mandato era in essere → eventuale diritto a provvigione. Ref: ${im.ref||_archiviaIdx}`,
        _immIdx:_archiviaIdx, _type:'verifica_provvigione'
      });
    }
  })();

  // ── [PROMEMORIA] Attività "Invia report visite al proprietario" ──
  //    Solo se l'immobile ha visite registrate (altrimenti non c'è nulla da inviare).
  (function(){
    if(!Array.isArray(D.attivita)) D.attivita=[];
    var nVis=0;
    try{
      nVis=D.visite.filter(function(v){
        return (typeof _visAppartieneImm==='function')
          ? _visAppartieneImm(v,_archiviaIdx)
          : parseInt(v.immRef)===_archiviaIdx;
      }).length;
    }catch(e){}
    // evita doppioni se si ri-archivia
    D.attivita=D.attivita.filter(function(a){ return !(a && a._immRef===_archiviaIdx && a._auto==='report_archivio'); });
    if(nVis>0){
      var immDesc=(im.tipo||'Immobile')+(im.comune?' — '+im.comune:'')+(im.ref?' (Rif.'+im.ref+')':'');
      D.attivita.push({
        tipo:'Invio scheda immobile',
        stato:'Da fare',
        oggetto:'Invia report visite al proprietario — '+immDesc+' ('+nVis+' visit'+(nVis===1?'a':'e')+')',
        cliente:im.contatto||'',
        tel:im.telefono||'',
        immRef:String(_archiviaIdx),
        scadenza:(data||today()),
        note:'Immobile archiviato ('+motivoLabel+'). Inviare al proprietario il resoconto delle visite effettuate durante il mandato.',
        _immRef:_archiviaIdx,
        _auto:'report_archivio'
      });
    }
  })();
  ...
  // [PROMEMORIA] Se ci sono visite, proponi di aprire SUBITO il report pronto da inviare.
  (function(){
    var idxReport=_archiviaIdx;
    var nVis=0;
    try{
      nVis=D.visite.filter(function(v){
        return (typeof _visAppartieneImm==='function')?_visAppartieneImm(v,idxReport):parseInt(v.immRef)===idxReport;
      }).length;
    }catch(e){}
    if(nVis>0 && typeof dlgConfirm==='function' && typeof openReportVisite==='function'){
      setTimeout(function(){
        dlgConfirm(
          'Questo immobile ha '+nVis+' visit'+(nVis===1?'a':'e')+' registrat'+(nVis===1?'a':'e')+'.\nVuoi aprire ora il report da inviare al proprietario?\n\n(Un promemoria è stato comunque aggiunto in Attività.)',
          '','Report visite pronto'
        ).then(function(ok){
          if(ok){
            openReportVisite(idxReport);
            setTimeout(function(){
              try{
                var sel=document.getElementById('report-periodo');
                if(sel){ sel.value='tutte'; if(typeof renderReportVisite==='function') renderReportVisite(); }
              }catch(e){}
            }, 50);
          }
        });
...
function riattiviaImmobile(i){
  ...
  dlgConfirm(
    ...
    // [PROVVIGIONE] rimuovi le 3 scadenze di verifica create all'archiviazione
    D.eventi=D.eventi.filter(e=>!(e._immIdx===i && e._type==='verifica_provvigione'));
...
function delImmobile(i){
  ...
  const im=D.immobili[i];
  if(!im) return;
  const nVis=D.visite.filter(v=>(typeof _visAppartieneImm==="function"?_visAppartieneImm(v,i):parseInt(v.immRef)===i)).length;
  ...
  D.visite=D.visite.filter(v=>!(typeof _visAppartieneImm==="function"?_visAppartieneImm(v,i):parseInt(v.immRef)===i));
      ...
      return`<div class="imm-card">
        ...
        <div class="imm-body">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            ${(function(){
              // [FIX CARD] Per venduto/archiviato il badge sulla foto basta:
              // niente tag di stato duplicato e niente tag incarico ("vendita") ridondante.
              var st=(im.stato||'').toLowerCase();
              if(st==='venduto'||st==='affittato'||st==='archiviato'||st==='non attivo'){
                return im._archiviaData||im._venditaData
                  ? '<span style="font-size:0.68rem;color:var(--text4)">'+(im._venditaData?'Venduto il '+fmtD(im._venditaData):'Archiviato il '+fmtD(im._archiviaData))+'</span>'
                  : '<span></span>';
              }
              return bStato(im.stato)+'<span class="badge badge-blue" style="font-size:0.68rem">'+(im.incarico||'')+'</span>';
            })()}
          </div>
...
function renderSchedaImmobile(i){
  ...
  const visite=D.visite.filter(v=>(typeof _visAppartieneImm==="function"?_visAppartieneImm(v,i):parseInt(v.immRef)===i)).sort((a,b)=>(a.data||'').localeCompare(b.data||''));
