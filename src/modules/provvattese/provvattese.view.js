...
(function(){
  ...
  function _calcola(){
    ...
    prat.forEach(function(p){
      ...
      var _teorico = (_pa>0) ? 0 : importo*_n(q.accTeorica)/100;
      ...
      var v={ prop:(im&&im.contatto)||p.venditore||'', titolo:_nomeImm(im, p.descr),
        ...
        valore:_lordo, netto:_netto(_lordo,q), reale:false,
    ...
    imm.forEach(function(im, _ii){
      ...
      var _teoP = (_sa!==null) ? 0 : _valA;   /* se l'hai indicata non è più un'ipotesi */
      ...
      var _valTot = _valV + ((_sa!==null) ? _valA : 0);
  ...
  function _quadro(G){
    ...
    return '<div class="card" style="padding:14px;margin-bottom:14px">'
      ...
      +   '<div><div style="font-size:0.68rem;font-weight:700;color:var(--text4);text-transform:uppercase;letter-spacing:.5px">Su cui puoi contare</div>'
      +   '<div style="font-size:0.74rem;color:var(--text3)">maturato + proposte accettate · ogni immobile conta una volta sola</div>'
      +   (mut.lordo>0 ? '<div style="font-size:0.72rem;font-weight:700;color:#B45309;margin-top:2px">di cui '+_eur(mut.lordo)+' non ancora al sicuro (mutuo o riserva del venditore)</div>' : '')
      +   '</div>'
      +   '<div style="text-align:right"><div style="font-size:1.5rem;font-weight:900;color:var(--text);line-height:1.1">'+_eur(certo.lordo)+'</div>'
      +   '<div style="font-size:0.84rem;font-weight:800;color:#15803D">a te '+_eur(certo.netto)+'</div></div>'
      + '</div>'
      ...
      + '</div>'
      + '<div style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;font-size:0.72rem;color:var(--text4);line-height:1.5">'
      +   '<span style="color:var(--text2)">La tua quota è <strong>'+q.agente+'%</strong> meno <strong>'+q.ufficio+'%</strong> di ufficio, applicata solo dove la provvigione non è ancora registrata (voci con <strong>*</strong>). '
      +     'La terza casella è la provvigione che chiedi di solito all\'acquirente (<strong>'+q.accTeorica+'%</strong>): dove non l\'hai ancora pattuita, la riga mostra in viola quanto varrebbe.</span>'
      +   '<span style="display:inline-flex;gap:5px;align-items:center">'
      +     '<input id="pa-q-agente" class="finput" style="width:58px;padding:4px 6px;text-align:right" value="'+q.agente+'">'
      +     '<input id="pa-q-ufficio" class="finput" style="width:58px;padding:4px 6px;text-align:right" value="'+q.ufficio+'">'
      +     '<input id="pa-q-acqt" class="finput" style="width:58px;padding:4px 6px;text-align:right" value="'+q.accTeorica+'" title="Provvigione che chiedi di solito all\'acquirente: serve a stimare gli affari dove non l\'hai ancora pattuita">'
      +     '<button onclick="_paSalvaQuote()" class="btn btn-outline btn-sm" style="padding:4px 9px">Ricalcola</button>'
      +   '</span>'
      + '</div></div>';
  }
  ...
  function _contenutoTab(G, quale){
    if(quale==='scadenze'){
      ...
      return '<div style="font-size:0.79rem;color:var(--text2);margin-bottom:8px">Incarichi in scadenza entro 90 giorni senza una proposta accettata. Una trattativa aperta non basta a metterli al sicuro.'
        + (_perdo>0 ? '<br><strong style="color:#DC2626">Se li perdi tutti se ne vanno '+_eur(_perdo)+'</strong>'
            + (_perdoT>0 ? ' <span style="color:#6D28D9;font-style:italic">più '+_eur(_perdoT)+' di parte acquirente — in tutto '+_eur(_perdo+_perdoT)+'</span>' : '') : '')
        + '</div>'
  ...
  window.apriProvvAttese=function(){
    ...
    w.innerHTML='<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--bg2);border-bottom:1px solid var(--border);flex-shrink:0">'
      ...
      + '<div style="flex:1;overflow:auto;padding:14px"><div style="max-width:900px;margin:0 auto">'
      +   _quadro(G)
