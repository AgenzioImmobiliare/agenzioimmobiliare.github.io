...
/* [17 set 2026] QUALE PROVVIGIONE HA GENERATO QUESTA SCRITTURA.
   Prima si leggeva solo "Prov#N" nella descrizione, cioè la POSIZIONE della
   provvigione quando la scrittura è nata. Le posizioni scivolano quando si
   cancella una provvigione: l'incasso NF del Ref.0038 (Prov#9) apriva la
   provvigione del Ref.0001, che oggi è la nona. Ora conta la targa
   (_provUuid); il Prov#N resta solo per le scritture che la targa non ce
   l'hanno. Se la targa c'è ma la provvigione non si trova, non si tira a
   indovinare con la posizione. */
function _nfProvIdx(n){
  const PR=D.provvigioni||[];
  if(n && n._provUuid){
    return PR.findIndex(p=>p&&p.uuid===n._provUuid);
  }
  const m=String((n&&n.descr)||'').match(/Prov#(\d+)/);
  return m?parseInt(m[1],10)-1:-1;
}
/* "Prov#1" è contenuto anche in "Prov#12": il riferimento va confrontato
   come numero intero, non come pezzo di testo. */
function _haRifProv(testo, numero){
  return new RegExp('Prov#'+numero+'(?!\\d)').test(String(testo||''));
}
...
function openNFModal(idxOrUuid){
  ...
  if(idx!==undefined){
    ...
    if(isAutoNF && banner && infoEl){
      const pvIdx=_nfProvIdx(n);
      const pv=pvIdx>=0?D.provvigioni[pvIdx]:null;
      const pvApri=(pv&&pv.uuid)?pv.uuid:pvIdx;
      if(pv){
        ...
        if(gotoBtn){
          gotoBtn.onclick=()=>{closeModal('modal-nf');setTimeout(()=>{go('provvigioni');setTimeout(()=>openProvModal(pvApri),300);},200);};
        }
      } else {
...
function saveNF(){
  ...
  if(_nfEditIdx!==null){
    ...
    /* [17 set 2026] Il record veniva ricostruito da zero con i soli campi del
       modulo: la targa _provUuid (e ogni altro campo tecnico) spariva al
       primo salvataggio, e la scrittura tornava a dipendere dalla posizione. */
    D.incassiNonFatt[_nfEditIdx]=(typeof aggiornaRecord==='function')
      ? aggiornaRecord(existingNF, n)
      : Object.assign({}, existingNF||{}, n);
  } else {
  ...
  if(wasEdit){
    ...
    if(isAuto){
      /* la targa sta sul record di prima: quello costruito dal modulo non la
         porta, e l'indice in modifica può essere già stato azzerato */
      const pvIdx=_nfProvIdx(existingNF);
      if(pvIdx>=0){
        ...
        if(pv){
          ...
          /* [17 set 2026] la data del pagamento ora resta: senza, la riga
             della provvigione perdeva la data a ogni modifica da qui */
          ...
          setTimeout(()=>{go('provvigioni');setTimeout(()=>openProvModal(pv.uuid||pvIdx),300);},200);
          return;
        }
      }
    }
    ...
    if(n){
      ...
      if(isAutoAg||isAutoAgt){
        D.provvigioni.forEach((pv,pvIdx)=>{
          /* [17 set 2026] con la targa si confronta SOLO la targa */
          const match = n._provUuid ? (!!pv && pv.uuid===n._provUuid) : (!!pv && _haRifProv(n.descr, pvIdx+1));
          if(match){
...
function _renderQuadraturaNF(){
  ...
  PR.forEach((p,pi)=>{
    ...
    const mie=NF.filter(n=>{
      ...
      return !n._provUuid && _haRifProv(n.descr, pi+1);
    });
      ...
      return`<tr ondblclick="${(n.note||'').includes('Auto da Provvigioni')?'':(`openNFModal('${nUuid}')`)}" style="cursor:pointer" title="${(n.note||'').includes('Auto da Provvigioni')?'Modificabile dalla provvigione collegata':'Doppio click per modificare'}">
        ...
        <td><div class="actions-col">
          ${(n.note||'').includes('Auto da Provvigioni')
            ? (() => {
                const pvIdx=_nfProvIdx(n);
                const pvL=pvIdx>=0?D.provvigioni[pvIdx]:null;
                /* si apre per codice: una posizione scritta nel pulsante
                   può essere già vecchia quando lo premi */
                const arg=(pvL&&pvL.uuid)?("'"+pvL.uuid+"'"):pvIdx;
                return pvL
                  ? `<button class="icon-btn" style="color:var(--brand);font-size:0.72rem;white-space:nowrap" onclick="closeModal('modal-nf');setTimeout(()=>{go('provvigioni');setTimeout(()=>openProvModal(${arg}),300);},200)" title="Modifica dalla provvigione collegata">↩️ Prov.</button>`
                  : `<span style="font-size:0.7rem;color:var(--red-l)" title="La provvigione di questa scrittura non si trova più">prov. mancante</span>`;
              })()
