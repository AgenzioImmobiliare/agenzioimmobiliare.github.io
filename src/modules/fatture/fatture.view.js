    ...
    if(ft){
      ...
      if(isAutoAg||isAutoAgt){
        D.provvigioni.forEach(pv=>{
          /* [17 set 2026] con la targa si confronta SOLO la targa; senza,
             il Prov#N va letto come numero intero (Prov#1 non è Prov#12) */
          const num=D.provvigioni.indexOf(pv)+1;
          const match = ft._provUuid
            ? (!!pv && pv.uuid===ft._provUuid)
            : (!!pv && new RegExp('Prov#'+num+'(?!\\d)').test(String(ft.descr||ft.descrizione||'')));
          if(match){
