...
function addFattVoce(descr,imp,provUuid){
  const list=document.getElementById('fatt-voci-list');
  if(!list)return;
  const row=document.createElement('div');
  row.className='fatt-voce-row';
  /* [24 set 2026] la voce ricorda a quale provvigione appartiene. Prima il
     modulo leggeva solo descrizione e importo: il collegamento si sarebbe
     perso al primo salvataggio (lo stesso guasto del 25 agosto sulla targa
     della fattura). */
  row.dataset.prov=provUuid||'';
  ...
  del.type='button'; del.className='fatt-voce-del'; del.title='Rimuovi';
  del.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
  del.addEventListener('click',function(){row.remove();calcFattura();
    try{ if(typeof window._fattAggiornaVaSu==='function') window._fattAggiornaVaSu(); }catch(e){}});
...
function getFattVoci(){
  ...
  rows.forEach(r=>{
    ...
    const v={descr:d,imp};
    if(r.dataset && r.dataset.prov) v._provUuid=r.dataset.prov;
    voci.push(v);
  });
  return voci;
}
function setFattVoci(voci){
  ...
  if(!list)return;
  list.innerHTML='';
  (voci||[]).forEach(v=>addFattVoce(v.descr,v.imp?fmtE(v.imp).replace('€','').trim():'',v._provUuid));
}
...
/* [24 set 2026] L'IMMOBILE DI UNA FATTURA.
   Prima la fattura salvava la POSIZIONE dell'immobile nella lista
   (immRef = 23 voleva dire "il ventiquattresimo immobile"). Quando la lista
   cambia ordine — il cloud unisce i due computer, si cancella un immobile —
   la stessa posizione indica un'altra casa: la 001/2026 mostrava (0023)
   Villa Agropoli invece di Capaccio e Giungano.
   Ora, in ordine di affidabilità:
   1) se la fattura è agganciata a provvigioni, l'immobile è quello delle
      provvigioni (per uuid, poi per codice Ref. scritto nella descrizione);
      una fattura accorpata ne mostra più d'uno;
   2) altrimenti l'immobile scelto a mano, salvato per uuid (immUuid);
   3) solo per le fatture vecchie, la posizione — segnalata come da verificare. */
function _fattImmobiliDi(f){
  const I=Array.isArray(D.immobili)?D.immobili:[], P=Array.isArray(D.provvigioni)?D.provvigioni:[];
  const targhe=[];
  if(f && f._provUuid) targhe.push(f._provUuid);
  ((f&&f.voci)||[]).forEach(v=>{ if(v&&v._provUuid&&targhe.indexOf(v._provUuid)<0) targhe.push(v._provUuid); });
  if(targhe.length){
    const lista=[]; let dubbi=false;
    targhe.forEach(u=>{
      const p=P.find(x=>x&&x.uuid===u); if(!p) return;
      let im=null;
      if(p.immUuid) im=I.find(x=>x&&x.uuid===p.immUuid)||null;
      if(!im){
        const m=String(p.descr||'').match(/\(Ref\.\s*([^)]+)\)/i);
        if(m){ const cod=m[1].trim(); im=I.find(x=>x&&String(x.ref||'').trim()===cod)||null; }
      }
      if(!im && p.immRef!==undefined && p.immRef!==null && p.immRef!==''){
        im=I[parseInt(p.immRef)]||null; if(im) dubbi=true;
      }
      if(im && lista.indexOf(im)<0) lista.push(im);
    });
    if(lista.length) return {lista, origine:'provvigioni', dubbi};
  }
  if(f && f.immUuid){ const a=I.find(x=>x&&x.uuid===f.immUuid); if(a) return {lista:[a], origine:'scelto', dubbi:false}; }
  if(f && f.immRef!==undefined && f.immRef!==null && f.immRef!==''){
    const b=I[parseInt(f.immRef)]; if(b) return {lista:[b], origine:'vecchio', dubbi:true};
  }
  return {lista:[], origine:'', dubbi:false};
}
function _fattImmEtich(im){ return '('+(im.ref||'—')+') '+(im.tipo||'Immobile')+' — '+(im.comune||''); }
function _fattImmVal(im){ const i=(D.immobili||[]).indexOf(im); return im&&im.uuid ? im.uuid : ('pos:'+i); }
/* riga informativa sotto il menù immobili */
function _fattImmInfo(testo, colore){
  const sel=document.getElementById('fatt-imm-ref'); if(!sel) return;
  let el=document.getElementById('fatt-imm-info');
  if(!el){ el=document.createElement('div'); el.id='fatt-imm-info';
    el.style.cssText='font-size:0.8rem;line-height:1.45;margin-top:6px;padding:8px 10px;border-radius:8px;border:1px solid #CBD5E1;background:#F8FAFC;color:#1E293B';
    sel.parentNode.insertBefore(el, sel.nextSibling); }
  el.innerHTML=testo||''; el.style.display=testo?'':'none';
  if(colore) el.style.borderColor=colore;
}
window._fattImmobiliDi=_fattImmobiliDi;
/* al salvataggio: l'uuid è la verità, la posizione si ricalcola adesso solo
   per le parti vecchie del programma che la leggono ancora */
function _fattImmDaSelect(){
  const s=document.getElementById('fatt-imm-ref');
  const vecchia=(_fattEditIdx!==null && D.fatture[_fattEditIdx]) ? D.fatture[_fattEditIdx] : {};
  if(!s || s.style.display==='none') return {immRef:vecchia.immRef!==undefined?vecchia.immRef:'', immUuid:vecchia.immUuid||''};
  const v=s.value||'';
  if(!v) return {immRef:'', immUuid:''};
  if(v.indexOf('pos:')===0) return {immRef:v.slice(4), immUuid:''};
  const i=(D.immobili||[]).findIndex(x=>x&&x.uuid===v);
  return {immRef:i>=0?String(i):'', immUuid:v};
}

function openFatturaModal(idxOrUuid){
  const idx=idxOrUuid!==undefined?(typeof idxOrUuid==='string'?D.fatture.findIndex(x=>x&&x.uuid===idxOrUuid):idxOrUuid):undefined;
  if(idxOrUuid!==undefined && (idx===undefined || idx<0)){
    if(typeof showToast==='function') showToast('Fattura non trovata (forse aggiornata altrove). Aggiorno la lista...','','#DC2626');
    try{ renderFatture(); }catch(e){}
    return;
  }
  _fattEditIdx = idx !== undefined ? idx : null;
  if(idx!==undefined){
    if(!D.fatture[idx].uuid) D.fatture[idx].uuid=(typeof genUUID==='function')?genUUID():('fatt_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));
    _fattEditUuid=D.fatture[idx].uuid;
  } else {
    _fattEditUuid=null;
  }
  ...
  if(immSel) immSel.innerHTML = '<option value="">-- Nessuno --</option>' +
    D.immobili.map((im,i) => im ? `<option value="${im.uuid||('pos:'+i)}">(${im.ref||i}) ${im.tipo||''} — ${im.comune||''}</option>` : '').join('');
  ...
  if(idx !== undefined){
    ...
    document.getElementById('fatt-note').value = f.note || '';
    {
      ...
      const RI=_fattImmobiliDi(f);
      immRefSel.removeAttribute('disabled'); immRefSel.title='';
      immRefSel.style.background=''; immRefSel.style.color=''; immRefSel.style.fontWeight='';
      if(RI.origine==='provvigioni'){
        /* l'immobile lo dicono le provvigioni collegate: niente menù */
        immRefSel.style.display='none';
        _fattImmInfo('<b>Dalle provvigioni collegate:</b> '+RI.lista.map(_fattImmEtich).map(_escFatt).join(' · ')
          +(RI.dubbi?'<br><span style="color:#92400E">Una provvigione ha solo il vecchio riferimento per posizione: verifica che l\'immobile sia giusto.</span>':''),
          RI.dubbi?'#D97706':'#CBD5E1');
      } else {
        immRefSel.style.display='';
        immRefSel.value = RI.lista[0] ? _fattImmVal(RI.lista[0]) : '';
        _fattImmInfo(RI.origine==='vecchio'
          ? '<span style="color:#92400E">Riferimento salvato col vecchio sistema (posizione nella lista): può indicare la casa sbagliata. Controlla l\'immobile e salva.</span>' : '',
          '#D97706');
      }
    }
    ...
    document.getElementById('fatt-imm-ref').style.display='';
    _fattImmInfo('');
...
function saveFattura(){
  /* Anti-drift: ri-risolvi l'indice reale della fattura in modifica appena
     prima di leggere/scrivere D.fatture. */
  if(_fattEditUuid){
    const _freshIdx=D.fatture.findIndex(x=>x&&x.uuid===_fattEditUuid);
    if(_freshIdx<0){
      alert('Questa fattura non esiste più (probabilmente cancellata o aggiornata da un altro dispositivo). La lista viene aggiornata.');
      _fattEditIdx=null; _fattEditUuid=null;
      try{ closeModal('modal-fattura'); }catch(_e){}
      renderFatture();
      return;
    }
    _fattEditIdx=_freshIdx;
  }
  ...
  if(_fattEditIdx !== null){
    f.uuid=_fattEditUuid||D.fatture[_fattEditIdx].uuid||((typeof genUUID==='function')?genUUID():('fatt_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8)));
    /* [25 ago 2026] PRIMA QUI SI SOSTITUIVA IL RECORD PER INTERO.
       L'oggetto f viene costruito leggendo i campi del modulo, e nel modulo
       NON esistono i campi tecnici: _provUuid (il collegamento alla
       provvigione) veniva quindi DISTRUTTO a ogni salvataggio della fattura.
       Conseguenza vista dall'utente: salvando la provvigione il programma
       agganciava la fattura e scriveva la targa, poi apriva da solo la scheda
       della fattura; bastava salvarla e la targa spariva. Alla modifica
       successiva la fattura non era più riconoscibile e ne nasceva una NUOVA,
       da cancellare a mano ogni volta.
       Ora i campi tecnici (quelli che iniziano con _) vengono conservati dal
       record precedente, a meno che il modulo non ne fornisca uno proprio. */
    const _vecchio=D.fatture[_fattEditIdx]||{};
    Object.keys(_vecchio).forEach(function(k){
      if(k.charAt(0)!=='_') return;
      if(f[k]===undefined) f[k]=_vecchio[k];
    });
    D.fatture[_fattEditIdx] = f;
  } else {
    f.uuid=(typeof genUUID==='function')?genUUID():('fatt_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));
    D.fatture.push(f);
  }
...
function eseguiAccorpamento(){
  ...
  selFatt.forEach(ft=>{
    if(ft.voci&&ft.voci.length>0){
      /* [24 set 2026] ACCORPARE NON TAGLIA PIÙ IL FILO. Prima la fattura unita
         nasceva senza targa e le originali (che l'avevano) venivano
         cancellate: gli incassi registrati dopo non si potevano più
         attribuire a nessuna provvigione (caso 005/2026 DI.GI.). Ora ogni
         voce porta con sé la targa della sua provvigione. */
      ft.voci.forEach(v=>{
        const nv={descr:(ft.numero?ft.numero+' — ':'')+v.descr, imp:v.imp||0};
        const t=v._provUuid||ft._provUuid; if(t) nv._provUuid=t;
        vociAccorp.push(nv);
      });
    } else {
      const nv={descr:(ft.numero?ft.numero+' — ':'')+( ft.descr||ft.oggetto||'Prestazione'), imp:getImpFatturabile(ft)};
      if(ft._provUuid) nv._provUuid=ft._provUuid;
      vociAccorp.push(nv);
    }
  });
...
function delFattura(idxOrUuid){
  ...
  const i0=typeof idxOrUuid==='string'?D.fatture.findIndex(x=>x&&x.uuid===idxOrUuid):idxOrUuid;
  const ft0=D.fatture[i0];
  if(!ft0){
    if(typeof showToast==='function') showToast('Fattura non trovata (forse già cancellata altrove). Aggiorno la lista...','','#DC2626');
    try{ renderFatture(); }catch(e){}
    return;
  }
  if(!ft0.uuid) ft0.uuid=(typeof genUUID==='function')?genUUID():('fatt_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));
  const ftUuid=ft0.uuid; /* catturato ORA: la cancellazione avviene dopo la conferma, che richiede tempo */
  dlgConfirm('Eliminare la fattura '+(ft0.numero||'')+'?','','Elimina Fattura').then(ok=>{
    if(!ok) return;
    /* Anti-drift: ri-risolvi l'indice fresco dall'uuid proprio ora che
       l'utente ha confermato. */
    const i=D.fatture.findIndex(x=>x&&x.uuid===ftUuid);
    if(i<0){
      if(typeof showToast==='function') showToast('Fattura già cancellata o aggiornata altrove. Aggiorno la lista...','','#DC2626');
      renderFatture();
      return;
    }
    ...
    if(ft){
      ...
      if(isAutoAg||isAutoAgt){
        D.provvigioni.forEach(pv=>{
          ...
          const match = pv.uuid && ft._provUuid ? ft._provUuid===pv.uuid : (ft.descr||ft.descrizione||'').includes(ref);
          if(match){
...
function renderFatture(){
  ...
  tbody.innerHTML = f.length ? f.map((ft,i)=>{
    ...
    if(!ft.uuid) ft.uuid=(typeof genUUID==='function')?genUUID():('fatt_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8));
    const ftUuid = ft.uuid.replace(/'/g,"\\'");
    ...
    return `<tr style="${rowBg};cursor:pointer;font-size:0.78rem" ondblclick="openFatturaModal('${ftUuid}')" title="Doppio click per modificare">
      ...
      <td style="padding:4px 6px"><div class="actions-col" style="gap:2px">
        ...
        <button class="icon-btn" onclick="openFatturaModal('${ftUuid}')" title="Modifica" style="padding:4px"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="icon-btn" onclick="delFattura('${ftUuid}')" style="color:var(--red-l);padding:4px" title="Elimina"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg></button>
      </div></td>
    </tr>`;
...
function stampaFattura(idx){
  ...
  const _RI = _fattImmobiliDi(f);
  const _immStampa = _RI.lista.map(x=>'('+(x.ref||'')+') '+(x.comune||'')).join(' · ');
  ...
  <table class="services">
    ...
    <tbody>
      <tr>
        ...
        <td style="text-align:center;font-size:9pt;color:#64748B">${_immStampa}</td>
...
Object.assign(window, {
  getTotaleVoci, calcFattura, addFattVoce, _cliFattEnsure, apriArchivioCliFatt, chiudiArchivioCliFatt,
