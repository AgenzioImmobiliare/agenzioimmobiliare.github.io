...
function openAgenteModal(idxOrUuid){
  ...
  /* Popola il menù "Utente di login collegato" con gli utenti definiti in
     _adm_users. Il collegamento è STABILE: salva l'id utente (es. usr_001),
     non il nome. Serve al multi-agente per sapere con certezza quale agente
     corrisponde a chi è loggato. */
  try{ _agPopulateUtenti(); }catch(_e){}
  var _agUsrSel = document.getElementById('ag-usrref');
  if(_agUsrSel) _agUsrSel.value = '';
  if(idx!==undefined){
    ...
    var _uSel=document.getElementById('ag-usrref'); if(_uSel) _uSel.value=a.usrRef||'';
...
/* Popola il <select id="ag-usrref"> con gli utenti di login (_adm_users).
   Mostra nome + email; il valore è l'id stabile dell'utente (es. usr_001). */
function _agPopulateUtenti(){
  var sel=document.getElementById('ag-usrref');
  if(!sel) return;
  var utenti=[];
  try{ if(Array.isArray(_adm_users)) utenti=_adm_users; }catch(_e){}
  var opts='<option value="">— Nessuno —</option>';
  utenti.forEach(function(u){
    if(!u||!u.id) return;
    var lbl=(u.nome||u.email||u.id)+(u.email?(' ('+u.email+')'):'');
    opts+='<option value="'+u.id+'">'+lbl+'</option>';
  });
  sel.innerHTML=opts;
}
try{ window._agPopulateUtenti=_agPopulateUtenti; }catch(e){}
...
function saveAgente(){
  ...
  /* indice REALE in D.agenti (vedi nota in immobili.view.js): mappare sulla
     lista filtrata assegnava l'immobile all'agente sbagliato */
  if(immAgSel) immAgSel.innerHTML='<option value="">-- Seleziona agente --</option>'+D.agenti.map((ag,i)=>(ag&&ag.stato!=='non attivo')?`<option value="${i}">${ag.nome||('Agente '+(i+1))}</option>`:'').join('');
}
function openAgenteSchedaPopup(i){
  ...
  /* ── MATURATO vs INCASSATO ──────────────────────────────────────────────
     Maturato: quota agente delle provvigioni la cui DATA OPERAZIONE cade nel
       periodo (già in totAgenteLordo, calcolato su agProv filtrato per periodo).
     Incassato: somma dei PAGAMENTI datati (modAgenteRighe[].data) che cadono
       nel periodo — indipendentemente da quando l'operazione è maturata. Così
       "maturato nel periodo" e "incassato nel periodo" sono due numeri distinti. */
  const _maturato=totAgenteLordo;
  let _incassato=0, _incassatoTutto=0;
  agProvAll.forEach(p=>{
    const righe=Array.isArray(p.modAgenteRighe)?p.modAgenteRighe:[];
    righe.forEach(r=>{
      const imp=parseFloat(r.imp)||0; if(imp<=0) return;
      _incassatoTutto+=imp;
      const dPag=_isoDate(r.data||'');
      /* Se il pagamento ha una data, filtra per periodo; se non ce l'ha,
         lo includiamo solo nel totale "Tutto" (periodo senza limite). */
      if(!_minDate){ _incassato+=imp; }
      else if(dPag && dPag>=_minDate){ _incassato+=imp; }
    });
  });
  const totAgenzia=agProv.reduce((s,p)=>s+(parseFloat(p.quotaAgenzia)||0),0);
  const percUff=parseFloat(a.quotaUfficio)||2.5;
  ...
  ov.innerHTML=`<div class="modal" style="max-width:900px;width:96%">
    ...
    <div class="mbody" style="max-height:82vh;overflow-y:auto;padding:18px 20px">
      ...
      <!-- MATURATO vs INCASSATO nel periodo -->
      <div style="display:flex;gap:10px;margin-bottom:14px;flex-wrap:wrap">
        <div style="flex:1;min-width:180px;background:linear-gradient(135deg,#EFF6FF,#DBEAFE);border:1px solid #BFDBFE;border-radius:10px;padding:11px 14px">
          <div style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#1E40AF">Quota Maturata nel periodo</div>
          <div style="font-size:1.25rem;font-weight:800;color:#1D4ED8;margin-top:2px">${fmtE(_maturato)}</div>
          <div style="font-size:0.66rem;color:#3730A3;margin-top:1px">quota agente da operazioni concluse</div>
        </div>
        <div style="flex:1;min-width:180px;background:linear-gradient(135deg,#F0FDF4,#DCFCE7);border:1px solid #BBF7D0;border-radius:10px;padding:11px 14px">
          <div style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#166534">Quota Incassata nel periodo</div>
          <div style="font-size:1.25rem;font-weight:800;color:#15803D;margin-top:2px">${fmtE(_incassato)}</div>
          <div style="font-size:0.66rem;color:#166534;margin-top:1px">pagamenti effettivamente ricevuti</div>
        </div>
        ${(_maturato-_incassato)>0.5?`<div style="flex:1;min-width:160px;background:linear-gradient(135deg,#FEF2F2,#FEE2E2);border:1px solid #FECACA;border-radius:10px;padding:11px 14px">
          <div style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#991B1B">Ancora da incassare</div>
          <div style="font-size:1.25rem;font-weight:800;color:#DC2626;margin-top:2px">${fmtE(_maturato-_incassato)}</div>
          <div style="font-size:0.66rem;color:#991B1B;margin-top:1px">maturato non ancora liquidato</div>
        </div>`:''}
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px">
        <div style="background:var(--bg2);border-radius:12px;padding:14px 16px;border:1px solid var(--border)">
          ...
          ${agProv.length>0?(()=>{
            const incassato=agProv.reduce((s,p)=>s+(parseFloat(p.quotaAgente)||0)*((p.statoPag==='Incassata')?1:0),0);
            const totQuota=agProv.reduce((s,p)=>s+(parseFloat(p.quotaAgente)||0),0);
            const tassoInc=totQuota>0?Math.round(incassato/totQuota*100):0;
            const ticket=agProv.length>0?totQuota/agProv.length:0;
            const nInc=agProv.filter(p=>p.statoPag==='Incassata').length;
            return `<div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border)">
              <div style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text4);margin-bottom:8px">Performance incassi</div>
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
                <div style="flex:1;background:#E8EEF5;border-radius:6px;height:18px;overflow:hidden;position:relative">
                  <div style="height:100%;width:${tassoInc}%;background:linear-gradient(90deg,#10B981,#059669);border-radius:6px;transition:width .5s"></div>
                </div>
                <span style="font-size:0.78rem;font-weight:800;color:#059669;min-width:38px;text-align:right">${tassoInc}%</span>
              </div>
              <div style="display:flex;gap:8px">
                <div style="flex:1;text-align:center;background:white;border:1px solid var(--border);border-radius:8px;padding:7px 4px">
                  <div style="font-size:0.9rem;font-weight:800;color:#1E293B">${fmtE(ticket)}</div>
                  <div style="font-size:0.58rem;color:var(--text4);text-transform:uppercase;letter-spacing:.3px">Quota media/op.</div>
                </div>
                <div style="flex:1;text-align:center;background:white;border:1px solid var(--border);border-radius:8px;padding:7px 4px">
                  <div style="font-size:0.9rem;font-weight:800;color:#059669">${nInc}/${agProv.length}</div>
                  <div style="font-size:0.58rem;color:var(--text4);text-transform:uppercase;letter-spacing:.3px">Op. incassate</div>
                </div>
              </div>
            </div>`;
          })():''}
        ...
        <div style="background:var(--bg2);border-radius:12px;padding:14px 16px;border:1px solid var(--border)">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;gap:8px">
            ...
            <span style="font-size:0.68rem;font-weight:700;color:#1D4ED8;background:#EFF6FF;padding:2px 9px;border-radius:12px">${fmtE(agProv.reduce((s,p)=>s+(parseFloat(p.quotaAgente)||0),0))} totale</span>
          </div>
          ${agProv.length>0?`
          <!-- BARRE ORIZZONTALI PER ANNO (vista principale, compatta) -->
          ${perAnno.length>0?`
          <div style="display:flex;flex-direction:column;gap:8px">
            ${perAnno.map(({anno,tot,n})=>{
              const isSel=anno===annoSel;
              return `<div onclick="window._agAnnoSel='${anno}';openAgenteSchedaPopup(${i})" title="Clicca per il dettaglio mensile di ${anno}" style="cursor:pointer;padding:2px 0">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:3px">
                  <span style="font-size:0.8rem;font-weight:800;color:${isSel?'#1D4ED8':'var(--text2)'}">${anno}${isSel?' ▾':''}</span>
                  <span style="font-size:0.78rem;font-weight:800;color:#15803D">${fmtE(tot)} <span style="font-size:0.62rem;color:var(--text4);font-weight:600">· ${n} op.</span></span>
                </div>
                <div style="background:#E8EEF5;border-radius:6px;height:20px;overflow:hidden">
                  <div style="height:100%;width:${Math.max(Math.round(tot/maxAnno*100),tot>0?4:0)}%;background:linear-gradient(90deg,${isSel?'#2563EB,#1D4ED8':'#60A5FA,#3B82F6'});border-radius:6px;transition:width .5s;box-shadow:${isSel?'0 1px 4px rgba(37,99,235,.35)':'none'}"></div>
                </div>
              </div>`;
            }).join('')}
          </div>`:''}
          <!-- DETTAGLIO MENSILE dell'anno selezionato (compatto) -->
          ${totAnnoSel>0?`
          <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
              <span style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text4)">Dettaglio mesi ${annoSel}</span>
              <span style="font-size:0.66rem;color:var(--text3)">Miglior mese: <strong style="color:#10B981">${mesi[meseMax]}</strong></span>
            </div>
            <div style="display:flex;align-items:flex-end;gap:2px;height:52px;background:white;border-radius:8px;padding:5px 7px 3px;border:1px solid var(--border)">
              ${barChart}
            </div>
          </div>`:''}
          <!-- ULTIME OPERAZIONI -->
          <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">
            <div style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text4);margin-bottom:6px">Ultime operazioni</div>
            ${agProv.slice(-4).reverse().map(p=>{const im=p.immRef!==undefined?D.immobili[parseInt(p.immRef)]:null;return`<div style="display:flex;align-items:center;gap:8px;padding:6px 9px;background:white;border-radius:7px;border:1px solid var(--border);margin-bottom:4px;font-size:0.76rem">
              ...
              <span style="padding:1px 6px;border-radius:8px;font-size:0.62rem;background:${p.statoPag==='Incassata'?'#DCFCE7':'#FEF2F2'};color:${p.statoPag==='Incassata'?'#15803D':'#DC2626'};white-space:nowrap">${p.statoPag==='Incassata'?'✓ Incassata':'⏳ In attesa'}</span>
            </div>`;}).join('')}
          </div>`:'<div class="empty-state" style="padding:24px 20px;text-align:center"><div style="font-size:2rem;opacity:.4;margin-bottom:6px">📊</div><p style="font-size:0.82rem;color:var(--text3)">Nessuna provvigione nel periodo selezionato</p><p style="font-size:0.72rem;color:var(--text4);margin-top:4px">Prova a selezionare "Tutto" o un periodo più ampio</p></div>'}
        </div>
      </div>
...
/* ═══════════════════════════════════════════════════════════════════
   _agReindexDopoElim(uuidEliminato)
   Dopo uno splice su D.agenti gli indici scalano di uno: ogni immobile,
   pratica o provvigione che puntava a un agente SUCCESSIVO a quello
   eliminato finirebbe silenziosamente sull'agente sbagliato.
   Qui l'uuid è la verità: gli indici vecchi (agenteRef / agenteIdx)
   vengono RICALCOLATI dall'uuid, e i record dell'agente eliminato
   restano senza agente invece di essere attribuiti a un altro.
   Presuppone che i record siano già ancorati (riparaCollegamenti).
   ═══════════════════════════════════════════════════════════════════ */
function _agReindexDopoElim(uuidEliminato){
  var mapU={};
  (D.agenti||[]).forEach(function(a,i){ if(a&&a.uuid) mapU[a.uuid]=i; });
  var rimappa=function(arr, campoIdx, vuoto){
    var orfani=0;
    (arr||[]).forEach(function(r){
      if(!r) return;
      if(r.agenteUuid && uuidEliminato && r.agenteUuid===uuidEliminato){
        r.agenteUuid=''; r[campoIdx]=vuoto; orfani++; return;
      }
      if(r.agenteUuid && mapU[r.agenteUuid]!==undefined){
        r[campoIdx]=(campoIdx==='agenteRef')?String(mapU[r.agenteUuid]):mapU[r.agenteUuid];
        return;
      }
      /* nessun uuid: non si può sapere chi fosse davvero. Meglio lasciarlo
         senza agente che attribuirlo per posizione a quello sbagliato. */
      if(r[campoIdx]!==undefined && r[campoIdx]!==null && r[campoIdx]!==''){
        r[campoIdx]=vuoto; orfani++;
      }
    });
    return orfani;
  };
  var o=0;
  o+=rimappa(D.immobili,'agenteRef','');
  o+=rimappa(D.pratiche,'agenteIdx',undefined);
  o+=rimappa(D.provvigioni,'agenteIdx',undefined);
  o+=rimappa(D.visite,'agenteRef','');
  return o;
}
...
function delAgente(idxOrUuid){
  ...
  /* Prima di toccare la lista: ancora ogni record all'uuid dell'agente,
     altrimenti dopo lo splice non sarebbe più possibile sapere di chi era. */
  try{ if(typeof riparaCollegamenti==='function') riparaCollegamenti(true); }catch(e){}
  const uuidElim=(D.agenti[i]||{}).uuid||'';
  const _conta=function(arr,campoIdx){
    return (arr||[]).filter(function(r){
      if(!r) return false;
      if(r.agenteUuid) return uuidElim && r.agenteUuid===uuidElim;
      return String(r[campoIdx])===String(i);
    }).length;
  };
  const nProv=_conta(D.provvigioni,'agenteIdx');
  const nImm=_conta(D.immobili,'agenteRef');
  const nPrat=_conta(D.pratiche,'agenteIdx');
  const tot=nProv+nImm+nPrat;
  if(tot>0){
    if(!confirm('Questo agente ha dei collegamenti:\n\n'+
      '• '+nImm+' immobili\n• '+nPrat+' pratiche\n• '+nProv+' provvigioni\n\n'+
      'Eliminandolo questi record restano in archivio ma rimangono SENZA agente.\nProcedere?')) return;
  } else {
  ...
  const orfani=_agReindexDopoElim(uuidElim);
  if(orfani>0 && typeof showToast==='function'){
    showToast(orfani+' record sono rimasti senza agente: riassegnali quando vuoi.','','#D97706');
  }
