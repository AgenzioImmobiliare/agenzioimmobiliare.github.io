...
(function(){
  ...
  function _aggiornaCatasto(){
    ...
    img.onload=function(){
      try{
        var nuovo=L.imageOverlay(url, b, {opacity:0.8, interactive:false, className:'mcat-ov'});
        nuovo.addTo(_map);
  ...
  function _visPannello(){
    return '<div id="mcat-vis-box" style="display:none;margin-top:8px;padding:10px 12px;background:var(--bg2);'
      ...
      +   '<button class="btn btn-outline btn-sm" onclick="_mcatVisuraSalva()">Salva nella scheda</button>'
  ...
  /* [25 set 2026] SFONDO a scelta: cartina, satellite (Esri), ortofoto 2012.
     ...
     - Ortofoto AGEA 2012 dal Geoportale Nazionale: servizio documentato solo in
       http; qui si chiede in https. Se non risponde lo dice l'avviso giallo. */
  var BASI = {
    ...
    agea:  { nome:'Ortofoto 2012', crea:function(){
      return L.tileLayer.wms('https://wms.pcn.minambiente.it/ogc?map=/ms_ogc/WMS_v1.3/raster/ortofoto_colore_12.map',{
        layers:'OI.ORTOIMMAGINI.2012', format:'image/jpeg', transparent:false, version:'1.1.1',
        attribution:'Ortofoto AGEA 2012 — Geoportale Nazionale (MASE)', maxZoom:19 }); } }
  };
  ...
  window.apriMappaCatastale=function(immUuid){
    ...
    /* [25 set 2026] sfondo scelto l'ultima volta (cartina, satellite, ortofoto);
    ...
    if(!_el('mcat-stile-foto')){
      ...
      st.textContent='#mcat-map.mcat-foto .mcat-ov{opacity:1!important;filter:drop-shadow(0 0 1px #fff) drop-shadow(0 0 1px #fff)}';
