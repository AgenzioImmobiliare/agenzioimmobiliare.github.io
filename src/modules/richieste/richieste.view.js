...
/* [9 set 2026] Anno della richiesta: la data in cui è stata raccolta. */
function _riqAnno(r){
  var d=String((r&&r.data)||'').slice(0,4);
  return /^\d{4}$/.test(d)?d:'';
}
function _riqAggiornaAnni(){
  var sel=document.getElementById('f-riq-anno');
  if(!sel) return;
  var conta={};
  (D.richieste||[]).forEach(function(r){ var a=_riqAnno(r); if(a) conta[a]=(conta[a]||0)+1; });
...
function renderRichieste(){
  _riqAggiornaAnni();
  const _annoRiq=(document.getElementById('f-riq-anno')?.value||'');
  ...
  const f = D.richieste.filter(function(r){
    ...
    if(_annoRiq && _riqAnno(r)!==_annoRiq) return false;   /* [9 set 2026] filtro per anno */
