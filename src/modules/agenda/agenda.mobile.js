  ...
  if(!items.length){
    ...
    items.forEach(function(it){
      ...
      if(_telClean){
        ...
        _btnsEv.push('<a href="tel:'+_telFull+'" class="mob-vis-act-btn call" title="Chiama"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg></a>');
        _btnsEv.push('<a href="https://wa.me/'+_telFull+'" target="_blank" rel="noopener" class="mob-vis-act-btn wa" title="WhatsApp"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a10 10 0 00-8.5 15.3L2 22l4.8-1.4A10 10 0 1012 2zm0 18a8 8 0 01-4.1-1.1l-.3-.2-2.8.8.8-2.7-.2-.3A8 8 0 1112 20zm4.4-6c-.2-.1-1.4-.7-1.6-.8s-.4-.1-.5.1-.6.8-.8 1-.3.2-.5.1a6.5 6.5 0 01-1.9-1.2 7.3 7.3 0 01-1.4-1.7c-.1-.2 0-.4.1-.5l.4-.4.2-.4v-.4l-.8-1.8c-.2-.5-.4-.4-.5-.4h-.5a1 1 0 00-.7.3A2.8 2.8 0 006 8.9c0 1.7 1.2 3.3 1.4 3.5s2.4 3.7 5.8 5.1c2.9 1.1 2.9.8 3.4.7s1.6-.6 1.8-1.3.2-1.2.2-1.3-.2-.2-.4-.3z"/></svg></a>');
        _btnsEv.push('<a href="https://wa.me/'+_telFull+'?text='+encodeURIComponent(_promMsg)+'" target="_blank" rel="noopener" class="mob-vis-act-btn rem" title="Promemoria"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8a6 6 0 00-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg></a>');
      }
      /* [26 ago 2026] Profilo acquirente: solo sulle visite vere (le card
         azzurre). Sugli appuntamenti non ancora trasformati non c'è nessuna
         visita a cui attaccarlo, quindi il pulsante non compare.
         Icona piena e blu se il profilo c'è già, vuota e grigia se manca:
         scorrendo l'agenda si vede subito per quali visite manca il dato. */
      if(it.visId){
        _btnsEv.push('<a href="javascript:void(0)" onclick="event.preventDefault();event.stopPropagation();mobApriProfiloVisita(\''+it.visId+'\')" class="mob-vis-act-btn" '
          + 'title="'+(it.haProfilo?'Profilo acquirente raccolto — tocca per rivederlo':'Profilo acquirente non ancora raccolto')+'" '
          + 'style="background:'+(it.haProfilo?'#EFF6FF':'#F8FAFC')+';color:'+(it.haProfilo?'#1D4ED8':'#94A3B8')+';border:1px solid '+(it.haProfilo?'#BFDBFE':'#E2E8F0')+'">'
          + '<svg width="16" height="16" viewBox="0 0 24 24" fill="'+(it.haProfilo?'currentColor':'none')+'" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></a>');
      }
      if(it.indirizzo){
        _btnsEv.push('<a href="https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(it.indirizzo)+'" target="_blank" rel="noopener" class="mob-vis-act-btn maps" title="Apri in Maps"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg></a>');
      }
      var _actionsHtml = _btnsEv.length
        ? '<div class="mob-vis-actions" onclick="event.stopPropagation()">'+_btnsEv.join('')+'</div>'
        : '';
      /* [3 set 2026] La scheda ora si apre al tocco. Prima non aveva nessun
         gestore: reagivano solo i pulsanti, quindi dall'agenda del telefono
         non c'era modo di modificare un appuntamento. Gli eventi che arrivano
         da calendari esterni restano non toccabili: la sincronizzazione li
         riscrive e una modifica andrebbe persa. */
      var _apri = (it.modificabile && it.agendaKey !== null && it.agendaKey !== undefined)
        ? ' onclick="mobApriEventoDaAgenda('+it.agendaKey+')" style="border-left-color:'+it.col+';cursor:pointer"'
        : ' style="border-left-color:'+it.col+';"';
      /* Riga del periodo per gli appuntamenti lunghi: "dal 12 al 16 settembre". */
      var _periodoHtml = it.periodo
        ? '<div style="font-size:0.72rem;color:#64748B;font-weight:600;margin-top:2px">'+_mobEsc(it.periodo)+'</div>'
        : '';
      html += '<div class="mob-event"'+_apri+'>'
