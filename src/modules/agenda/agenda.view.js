...
function apRenderPipeline(){
  ...
  filtered.forEach(function(deal, idx){
    ...
    if(!isLost && !isWon && (_rit.n > 0 || _rit.viaFollowup)){
      ...
      _chipChiamate = '<span title="'+(_rit.viaFollowup
    ...
    AP_PIPELINE_STAGES.forEach(function(stage, si){
      ...
      (function(sid, corrente){
        cell.onclick = function(e){
          e.stopPropagation();
          if(corrente && !isLostStage) apRegistraChiamata(deal.id);
...
/* Registra una chiamata nella fase corrente. Chiede sempre conferma:
   un clic di troppo gonfierebbe un numero su cui poi decidi chi richiamare. */
function apRegistraChiamata(dealId){
  var deals = apGetDeals();
  var deal = deals.find(function(d){ return d.id===dealId; });
  if(!deal) return;
  var st = AP_PIPELINE_STAGES.find(function(x){ return x.id===deal.fase; });
  var ch = apChiamateFase(deal);
  var msg = 'Registrare una chiamata a <strong>'+String(deal.titolo||'').replace(/[&<>]/g,'')+'</strong>'
    + ' nella fase "'+((st&&st.label)||deal.fase)+'"?'
    + (ch.length ? ('<br><br>Finora in questa fase: <strong>'+ch.length+'</strong>'
        + (ch[ch.length-1].data ? (', l\'ultima il '+apFmtData(ch[ch.length-1].data)) : '')+'.')
      : '<br><br>Sarebbe la prima in questa fase.');
  var _fai = function(){
    if(!Array.isArray(deal.chiamate)) deal.chiamate = [];
    deal.chiamate.push({ data:new Date().toISOString().slice(0,10), fase:deal.fase });
    saveD();
    apRenderPipeline();
    var n = apChiamateFase(deal).length;
    showToast(n+'ª chiamata registrata','','#7C3AED');
  };
  if(typeof dlgConfirm==='function') dlgConfirm(msg,'','Registra chiamata').then(function(ok){ if(ok) _fai(); });
  else if(confirm('Registrare una chiamata a '+(deal.titolo||'')+'?')) _fai();
}
...
Object.assign(window, { apSvgColored, apGetDeals, apFmt, renderAgendaPipeline, apRenderKpi, apGetWeekDays, apWeekMove, apWeekToday, apRenderAgenda, apRenderPipeline, apEditDeal, apSaveDeal, apNuovoEvento, apModificaEvento, apNuovoDeal, apMoveDeal, apDelDeal,
  apChiamateFase, apGiorniDa, apRitardoChiamata, apRegistraChiamata, apFmtData });
