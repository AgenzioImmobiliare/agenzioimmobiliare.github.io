...
function apRenderKpi(){
  ...
  /* [3 set 2026] Eventi su più giorni: un corso o le ferie contano in tutti
     i giorni che coprono, non solo il primo. La regola sta nell'index
     (_evCopreGiorno); se manca si torna al confronto per data esatta. */
  var _copre=function(e,ds){ return (typeof window._evCopreGiorno==='function')
    ? window._evCopreGiorno(e,ds) : ((e.data||'').slice(0,10)===ds); };
  var todayEvts=evts.filter(function(e){return _copre(e,todayS);}).length;
  var week7=evts.filter(function(e){
...
function apRenderAgenda(){
  ...
  days.forEach(function(d,idx){
    ...
    /* [3 set 2026] Coperto anche dai giorni intermedi degli eventi lunghi.
       Restano gli OGGETTI ORIGINALI, non copie: più sotto il doppio clic
       ritrova l'evento con D.eventi.indexOf(evObj), e su una copia
       darebbe -1 aprendo l'evento sbagliato o nessuno. */
    var dayEvts=evts.filter(function(e){
        return (typeof window._evCopreGiorno==='function')
          ? window._evCopreGiorno(e,ds) : ((e.data||'').slice(0,10)===ds);
      })
    ...
    if(dayEvts.length===0){
      ...
      dayEvts.forEach(function(ev){
        ...
        if(ev._type === 'visita' && typeof ev._visIdx === 'number'){
          ...
          /* Passo l'INDICE reale in D.eventi (sempre univoco), oltre all'id.
             Così se due eventi hanno lo stesso id (o id mancante) si apre
             comunque quello giusto su cui hai cliccato. */
          (function(evObj){
            var realIdx = D.eventi ? D.eventi.indexOf(evObj) : -1;
            var evId = evObj.id;
            card.ondblclick=function(e){e.stopPropagation();apModificaEvento(evId, realIdx);};
          })(ev);
        }
        ...
        card.innerHTML=oraHtml
          ...
          +'<span style="font-size:0.71rem;font-weight:700;color:'+tc.color+';line-height:1.3;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+(ev.titolo||'Evento')
          /* [3 set 2026] Contatore "(3/5)" sugli eventi che durano più giorni.
             Aggiunto solo al disegno: il record in archivio non si tocca. */
          +((typeof window._evEtichettaGiorno==='function')?window._evEtichettaGiorno(ev,ds):'')+'</span></div>'
...
function apRenderPipeline(){
  ...
  filtered.forEach(function(deal, idx){
    ...
    var _rit = apRitardoChiamata(deal);
    var _chipChiamate = '';
    if(!isLost && !isWon && (_rit.n > 0 || _rit.viaFollowup)){
      var _col, _bg, _bd, _txt;
      if(_rit.scaduto){ _col='#B91C1C'; _bg='rgba(220,38,38,.10)'; _bd='rgba(220,38,38,.30)'; }
      else if(_rit.gg !== null && _rit.soglia && _rit.gg >= _rit.soglia-2){ _col='#B45309'; _bg='rgba(245,158,11,.12)'; _bd='rgba(245,158,11,.32)'; }
      else { _col='#15803D'; _bg='rgba(21,128,61,.10)'; _bd='rgba(21,128,61,.28)'; }
      _txt = (_rit.n>0 ? (_rit.n+(_rit.n===1?' chiamata':' chiamate')) : 'mai chiamato');
      if(_rit.gg !== null) _txt += ' · ' + (_rit.gg===0 ? 'oggi' : (_rit.gg===1 ? 'ieri' : _rit.gg+' gg fa'));
      /* [30 ago 2026] L'esito dell'ultima chiamata: sapere che hai provato
         tre volte serve poco se non si vede se hai parlato o no. */
      var _ultCh = apChiamateFase(deal);
      _ultCh = _ultCh.length ? _ultCh[_ultCh.length-1] : null;
      if(_ultCh && _ultCh.esito && AP_ESITI_CH[_ultCh.esito]) _txt += ' · ' + AP_ESITI_CH[_ultCh.esito].lbl.toLowerCase();
      if(_rit.viaFollowup && _rit.ggFollowup !== null){
        _txt += _rit.ggFollowup > 0 ? ' · richiamo scaduto' : (_rit.ggFollowup===0 ? ' · richiamo oggi' : '');
      }
      var _tip = apChiamateFase(deal).map(function(c){
        var e = AP_ESITI_CH[c.esito];
        return apFmtData(c.data)+' — '+((e&&e.lbl)||c.esito||'—')+(c.note?(' ('+c.note+')'):'');
      }).join('\n');
      _chipChiamate = '<span title="'+String((_rit.viaFollowup
            ? ('Richiamo fissato al '+apFmtData(deal.dataFollowup))
            : ('Da richiamare ogni '+_rit.soglia+' giorni (priorità '+((deal.priorita)||'media')+')'))
            + (_tip ? ('\n\n'+_tip) : '')).replace(/"/g,'&quot;')
          +'" style="display:inline-flex;align-items:center;gap:4px;font-size:0.59rem;font-weight:800;'
        + 'background:'+_bg+';color:'+_col+';border:1px solid '+_bd+';padding:1px 6px;border-radius:4px;white-space:nowrap">'
    ...
    infoCell.innerHTML = '<div style="font-size:0.82rem;font-weight:700;color:' + (isWon?'#065F46':'var(--text)') + ';white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'
      ...
      /* [30 ago 2026] Targhetta delle chiamate: quante in questa fase e da
         quanto aspetta. Compare solo se c'è qualcosa da dire — un contatto
         appena inserito non ha bisogno di un'etichetta in più. */
      +   _chipChiamate
      + '</div>';
    ...
    AP_PIPELINE_STAGES.forEach(function(stage, si){
      ...
      if(isCurrent && isLostStage){
        ...
        /* ── FASE ATTUALE: anello esterno + cerchio grande con gradiente ──
           [30 ago 2026] Se in questa fase ci sono chiamate registrate, il
           numero prende il posto dell'icona: è l'informazione che serve
           guardando la colonna, non il simbolo della fase che è già dato
           dalla posizione. Un pallino con "3" dice a colpo d'occhio quante
           volte hai già provato. */
        var _nCh = apChiamateFase(deal, stage.id).length;
        var _dentro = _nCh > 0
          ? '<span style="font-size:0.92rem;font-weight:900;color:white;line-height:1;text-shadow:0 1px 2px rgba(0,0,0,.25)">'+_nCh+'</span>'
          : AP_SVG[stage.svgKey].replace('stroke="currentColor"','stroke="white"');
      ...
      /* Click → cambia fase.
         [30 ago 2026] Se la fase è GIÀ quella, il clic non ha più niente da
         spostare: registra una chiamata (previa conferma). È il gesto che
         facevi già, che ora conta anche i tentativi. */
      (function(sid, corrente){
        cell.onclick = function(e){
          e.stopPropagation();
          if(corrente && sid === AP_FASE_CHIAMATE) apRegistraChiamata(deal.id);
          else if(corrente) return;              /* già in questa fase: niente da fare */
          else apMoveDeal(deal.id, sid);
        };
      })(stage.id, isCurrent);
      if(isCurrent && stage.id === AP_FASE_CHIAMATE){
        var _r0 = apRitardoChiamata(deal);
        cell.title = stage.label + ' (attuale) — clicca per registrare una chiamata'
          + (_r0.n ? ('\n'+_r0.n+' chiamat'+(_r0.n===1?'a':'e')+' in questa fase'
              + (_r0.ultima ? (', ultima il '+apFmtData(_r0.ultima)) : '')) : '\nNessuna chiamata registrata in questa fase')
          + (apChiamateFase(deal).length
              ? ('\n\nDate: '+apChiamateFase(deal).map(function(c){ return apFmtData(c.data); }).join(' · '))
              : '');
      }
...
function apNuovoEvento(preDate){
  ...
  D.eventi.push({id:'ev_ap_'+Date.now().toString(36)+Math.random().toString(36).slice(2,8),data:dataDefault,ora:ora||'',titolo:titolo.trim(),tipo:tipoFinal,cliente:cliente||'',note:note||''});
  saveD();
  renderAgendaPipeline();
  showToast('Evento aggiunto','','#15803D');
}

function apModificaEvento(evId, realIdx){
  if(!D.eventi)return;
  /* Identificazione robusta: se ho l'indice reale e punta a un evento con
     l'id atteso, uso quello (sicuro anche con id duplicati). Altrimenti
     ripiego sulla ricerca per id. */
  var ev=null, evIndex=-1;
  if(typeof realIdx==='number' && realIdx>=0 && D.eventi[realIdx] &&
     (D.eventi[realIdx].id===evId || !evId)){
    ev=D.eventi[realIdx]; evIndex=realIdx;
  } else {
    evIndex=D.eventi.findIndex(function(e){return e.id===evId;});
    ev=evIndex>=0?D.eventi[evIndex]:null;
  }
  if(!ev)return;
  ...
  if(!titolo.trim()){if(confirm('Eliminare questo evento?')){
    /* elimina per INDICE preciso (non per id, che potrebbe colpire un omonimo) */
    if(evIndex>=0){ D.eventi.splice(evIndex,1); }
    else { D.eventi=D.eventi.filter(function(e){return e!==ev;}); }
    saveD();renderAgendaPipeline();showToast('Evento eliminato','','#D97706');}return;}
...
/* ════════════════════════════════════════════════════════════════════
   [30 ago 2026] QUANTE VOLTE HO CHIAMATO IN QUESTA FASE
   Il problema: quando un contatto resta fermo su "Contattato", il pallino
   dice solo CHE è stato contattato, non QUANTE volte. Un cliente chiamato
   una volta e uno chiamato cinque volte sono indistinguibili, e senza quel
   numero non si capisce chi insistere e chi lasciar perdere.
   Le chiamate si registrano in deal.chiamate, ognuna con la data e la FASE
   in cui è avvenuta: così il conteggio riparte quando il contatto avanza,
   ed è sempre "quante volte l'ho chiamato in questa fase".
   ════════════════════════════════════════════════════════════════════ */
function apChiamateFase(deal, fase){
  var f = fase || (deal && deal.fase);
  var arr = (deal && Array.isArray(deal.chiamate)) ? deal.chiamate : [];
  return arr.filter(function(c){ return c && c.fase === f; });
}
function apGiorniDa(dataIso){
  if(!dataIso) return null;
  var t = new Date(String(dataIso)+'T00:00:00').getTime();
  if(isNaN(t)) return null;
  var oggi = new Date(); oggi.setHours(0,0,0,0);
  return Math.max(0, Math.round((oggi.getTime()-t)/86400000));
}
/* Ogni quanto è troppo: segue la priorità che stai già assegnando.
   Alta 7 giorni, Media 15, Bassa 30. Se hai scritto una data di follow-up
   nella scheda, comanda quella: una data decisa da te vale più di una
   regola generale. */
var AP_ATTESA = { alta:7, media:15, bassa:30 };
function apRitardoChiamata(deal){
  var ch = apChiamateFase(deal);
  var ultima = ch.length ? ch[ch.length-1].data : null;
  var gg = apGiorniDa(ultima);
  if(deal && deal.dataFollowup){
    var ggF = apGiorniDa(deal.dataFollowup);   /* positivo = scaduta */
    if(ggF !== null){
      return { n:ch.length, gg:gg, ultima:ultima, scaduto:(ggF>0), viaFollowup:true,
               soglia:null, ggFollowup:ggF };
    }
  }
  var soglia = AP_ATTESA[(deal&&deal.priorita)||'media'] || 15;
  return { n:ch.length, gg:gg, ultima:ultima,
           scaduto:(gg!==null && gg>soglia), viaFollowup:false, soglia:soglia };
}
function apFmtData(iso){
  var m = String(iso||'').match(/^(\d{4})-(\d{2})-(\d{2})/);
  return m ? (m[3]+'/'+m[2]+'/'+m[1]) : (iso||'');
}
/* Esiti possibili di una telefonata, con etichetta e colore. */
var AP_ESITI_CH = {
  'non-risponde' : { lbl:'Non risponde',   col:'#B45309', bg:'rgba(245,158,11,.12)', bd:'rgba(245,158,11,.32)' },
  'richiamare'   : { lbl:'Da richiamare',  col:'#1D4ED8', bg:'rgba(37,99,235,.10)',  bd:'rgba(37,99,235,.28)'  },
  'positivo'     : { lbl:'Positivo',       col:'#15803D', bg:'rgba(21,128,61,.10)',  bd:'rgba(21,128,61,.28)'  },
  'negativo'     : { lbl:'Negativo',       col:'#B91C1C', bg:'rgba(220,38,38,.10)',  bd:'rgba(220,38,38,.30)'  },
  'numero-errato': { lbl:'Numero errato',  col:'#64748B', bg:'rgba(100,116,139,.10)',bd:'rgba(100,116,139,.28)'}
};
/* La fase in cui ha senso contare le telefonate. Sulle altre il pallino
   continua a fare solo quello che faceva prima: spostare la fase.
   [30 ago 2026] Prima la richiesta compariva su OGNI fase attuale, anche
   dove una chiamata non c'entra niente. */
var AP_FASE_CHIAMATE = 'contattato';

/* Apre la scheda per registrare una telefonata: data, esito, note ed
   eventuale prossimo richiamo. Non basta contare i tentativi — serve sapere
   se hai parlato con qualcuno e com'è andata. */
function apRegistraChiamata(dealId){
  var deals = apGetDeals();
  var deal = deals.find(function(d){ return d.id===dealId; });
  if(!deal) return;
  var sv = function(id,v){ var e=document.getElementById(id); if(e) e.value=v; };
  sv('ap-ch-deal', dealId);
  apAnnullaModificaChiamata();
  sv('ap-ch-data', new Date().toISOString().slice(0,10));
  sv('ap-ch-esito', 'non-risponde');
  sv('ap-ch-note', '');
  sv('ap-ch-followup', deal.dataFollowup || '');
  var t = document.getElementById('ap-ch-titolo');
  if(t) t.textContent = deal.titolo || 'Registra chiamata';

  /* Storico delle chiamate già registrate in questa fase: mostrarlo qui
     evita di registrare due volte la stessa telefonata. */
  var box = document.getElementById('ap-ch-storico');
  if(box){
    var ch = apChiamateFase(deal);
    if(!ch.length){
      box.innerHTML = '<div style="font-size:0.76rem;color:var(--text3);background:var(--bg3);border:1px solid var(--border);border-radius:9px;padding:9px 12px">Nessuna chiamata ancora registrata in questa fase.</div>';
    } else {
      /* [30 ago 2026] Ogni riga porta i pulsanti per correggerla o toglierla.
         Il riferimento è la posizione REALE dentro deal.chiamate, non quella
         nell'elenco filtrato per fase: filtrando, le due non coincidono e si
         finirebbe per modificare la chiamata sbagliata. */
      var tutte = Array.isArray(deal.chiamate) ? deal.chiamate : [];
      var righe = [];
      tutte.forEach(function(c, i){ if(c && c.fase === deal.fase) righe.push({ c:c, i:i }); });
      box.innerHTML = '<div style="font-size:0.7rem;font-weight:800;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">'
        + ch.length+' chiamat'+(ch.length===1?'a':'e')+' in questa fase</div>'
        + '<div style="max-height:180px;overflow-y:auto;border:1px solid var(--border);border-radius:9px">'
        + righe.slice().reverse().map(function(r){
            var c = r.c;
            var e = AP_ESITI_CH[c.esito] || { lbl:(c.esito||'esito non indicato'), col:'var(--text3)', bg:'var(--bg3)', bd:'var(--border)' };
            return '<div style="display:flex;align-items:center;gap:8px;padding:7px 8px 7px 10px;border-bottom:1px solid var(--border)">'
              + '<span style="font-size:0.74rem;font-weight:700;white-space:nowrap">'+apFmtData(c.data)+'</span>'
              + '<span style="font-size:0.66rem;font-weight:800;color:'+e.col+';background:'+e.bg+';border:1px solid '+e.bd+';border-radius:5px;padding:1px 7px;white-space:nowrap">'+e.lbl+'</span>'
              + (c.note ? '<span style="font-size:0.7rem;color:var(--text3);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+String(c.note).replace(/[&<>]/g,'')+'</span>' : '')
              + '<span style="margin-left:auto;display:flex;gap:3px;flex-shrink:0">'
              +   '<button type="button" title="Correggi questa chiamata" onclick="apModificaChiamata(\''+deal.id+'\','+r.i+')" style="width:24px;height:24px;border-radius:6px;border:1px solid var(--border);background:var(--bg2);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text3)">'
              +     '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4z"/></svg></button>'
              +   '<button type="button" title="Elimina questa chiamata" onclick="apEliminaChiamata(\''+deal.id+'\','+r.i+')" style="width:24px;height:24px;border-radius:6px;border:1px solid var(--border);background:var(--bg2);cursor:pointer;display:flex;align-items:center;justify-content:center;color:#B91C1C">'
              +     '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg></button>'
              + '</span>'
              + '</div>';
          }).join('')
        + '</div>';
  ...
  openModal('modal-ap-chiamata');
}

/* Torna alla modalità "nuova chiamata": pulisce i campi e rimette
   l'etichetta giusta sul pulsante. */
function apAnnullaModificaChiamata(){
  var sv=function(id,v){ var e=document.getElementById(id); if(e) e.value=v; };
  sv('ap-ch-idx','');
  sv('ap-ch-data', new Date().toISOString().slice(0,10));
  sv('ap-ch-esito','non-risponde');
  sv('ap-ch-note','');
  var av=document.getElementById('ap-ch-modifica'); if(av) av.style.display='none';
  var bt=document.getElementById('ap-ch-salva'); if(bt) bt.textContent='Registra chiamata';
}
/* Carica una chiamata già registrata nei campi, per correggerla. */
function apModificaChiamata(dealId, idx){
  var deal = apGetDeals().find(function(d){ return d.id===dealId; });
  if(!deal || !Array.isArray(deal.chiamate)) return;
  var c = deal.chiamate[idx];
  if(!c){ showToast('Chiamata non trovata','','#D97706'); return; }
  var sv=function(id,v){ var e=document.getElementById(id); if(e) e.value=(v===undefined||v===null)?'':v; };
  sv('ap-ch-idx', String(idx));
  sv('ap-ch-data', c.data||'');
  sv('ap-ch-esito', c.esito||'non-risponde');
  sv('ap-ch-note', c.note||'');
  var av=document.getElementById('ap-ch-modifica');
  if(av){
    av.style.display='flex';
    var t=document.getElementById('ap-ch-modifica-txt');
    if(t) t.textContent='Stai correggendo la chiamata del '+apFmtData(c.data);
  }
  var bt=document.getElementById('ap-ch-salva'); if(bt) bt.textContent='Salva correzione';
}
/* Toglie una chiamata registrata per sbaglio. Chiede conferma: il numero
   che ne esce è quello su cui poi decidi se insistere. */
function apEliminaChiamata(dealId, idx){
  var deal = apGetDeals().find(function(d){ return d.id===dealId; });
  if(!deal || !Array.isArray(deal.chiamate)) return;
  var c = deal.chiamate[idx];
  if(!c) return;
  var e = AP_ESITI_CH[c.esito];
  var msg = 'Eliminare la chiamata del <strong>'+apFmtData(c.data)+'</strong>'
    + (e ? (' con esito "'+e.lbl+'"') : '') + '?'
    + (c.note ? ('<br><br>Nota: '+String(c.note).replace(/[&<>]/g,'')) : '')
    + '<br><br>Il conteggio delle chiamate scende di uno.';
  var _fai = function(){
    deal.chiamate.splice(idx,1);
    saveD();
    apRegistraChiamata(dealId);   /* ridisegna lo storico nella scheda aperta */
    apRenderPipeline();
    showToast('Chiamata eliminata','','#B91C1C');
  };
  if(typeof dlgConfirm==='function') dlgConfirm(msg,'','Elimina chiamata').then(function(ok){ if(ok) _fai(); });
  else if(confirm('Eliminare la chiamata del '+apFmtData(c.data)+'?')) _fai();
}

function apSalvaChiamata(){
  var gv = function(id){ var e=document.getElementById(id); return e ? String(e.value||'').trim() : ''; };
  var dealId = gv('ap-ch-deal');
  var deals = apGetDeals();
  var deal = deals.find(function(d){ return d.id===dealId; });
  if(!deal){ showToast('Contatto non trovato','','#D97706'); return; }
  var data = gv('ap-ch-data') || new Date().toISOString().slice(0,10);
  var esito = gv('ap-ch-esito') || 'non-risponde';
  var note = gv('ap-ch-note');
  var followup = gv('ap-ch-followup');

  /* [30 ago 2026] Se c'è una posizione in modifica si CORREGGE quella riga,
     altrimenti se ne aggiunge una nuova. */
  var idxTxt = gv('ap-ch-idx');
  var inModifica = idxTxt !== '' && !isNaN(parseInt(idxTxt));
  var idx = inModifica ? parseInt(idxTxt) : -1;
  if(inModifica && !(Array.isArray(deal.chiamate) && deal.chiamate[idx])){
    showToast('La chiamata da correggere non esiste più','','#D97706');
    apRegistraChiamata(dealId);
    return;
  }

  /* Stessa data e stesso esito già registrati: quasi sempre è un doppio
     salvataggio, non due telefonate identiche nello stesso giorno.
     In correzione non vale per la riga che si sta modificando. */
  var gia = !inModifica && apChiamateFase(deal).some(function(c){ return c.data===data && c.esito===esito; });
  var _scrivi = function(){
    if(!Array.isArray(deal.chiamate)) deal.chiamate = [];
    if(inModifica){
      deal.chiamate[idx] = { data:data, esito:esito, note:note, fase:deal.chiamate[idx].fase || deal.fase };
    } else {
      deal.chiamate.push({ data:data, esito:esito, note:note, fase:deal.fase });
    }
    if(followup) deal.dataFollowup = followup;
    saveD();
    closeModal('modal-ap-chiamata');
    apRenderPipeline();
    var n = apChiamateFase(deal).length;
    var e = AP_ESITI_CH[esito];
    showToast(inModifica ? ('Chiamata corretta — '+((e&&e.lbl)||esito))
                         : (n+'ª chiamata registrata — '+((e&&e.lbl)||esito)),'','#7C3AED');
  };
  if(gia && typeof dlgConfirm==='function'){
    dlgConfirm('Hai già registrato una chiamata con lo stesso esito il '+apFmtData(data)
      +'.<br><br>Vuoi registrarne un\'altra lo stesso?','','Chiamata già registrata')
      .then(function(ok){ if(ok) _scrivi(); });
  } else _scrivi();
}
...
Object.assign(window, { apSvgColored, apGetDeals, apFmt, renderAgendaPipeline, apRenderKpi, apGetWeekDays, apWeekMove, apWeekToday, apRenderAgenda, apRenderPipeline, apEditDeal, apSaveDeal, apNuovoEvento, apModificaEvento, apNuovoDeal, apMoveDeal, apDelDeal,
  apChiamateFase, apGiorniDa, apRitardoChiamata, apRegistraChiamata, apSalvaChiamata, apFmtData,
  apModificaChiamata, apEliminaChiamata, apAnnullaModificaChiamata });
