  ...
  if(box){
    ...
    if(!ch.length){
      ...
      box.innerHTML = '<div style="font-size:0.7rem;font-weight:800;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">'
        ...
        + '<div style="max-height:150px;overflow-y:auto;border:1px solid var(--border);border-radius:9px">'
        + ch.slice().reverse().map(function(c){
            var e = AP_ESITI_CH[c.esito] || { lbl:(c.esito||'—'), col:'var(--text3)', bg:'var(--bg3)', bd:'var(--border)' };
...
function apSalvaChiamata(){
  ...
  var gia = apChiamateFase(deal).some(function(c){ return c.data===data && c.esito===esito; });
  var _scrivi = function(){
    ...
    showToast(n+'ª chiamata registrata — '+((e&&e.lbl)||esito),'','#7C3AED');
  };
